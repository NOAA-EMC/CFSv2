...
C-----------------------------------------------------------------------
C-----------------------------------------------------------------------
      CALL W3TAGB('BUFR_QUIPC',2013,0022,0050,'NP22')

      print *
      print * ,'---> Welcome to BUFR_QUIPC - Version 01-22-2013'
      print *
...
C  -------------------------------------------------------------------
     ...
     .    5X,"NOT APPLIED TO DUMP FILE(S)"/)', LUEDT
         JRET = SYSTEM('[ -n "$NET" ]')
         IF(JRET.EQ.0)  THEN
            KRET = SYSTEM('[ "$NET" = gfs -o "$NET" = gdas ]')
            IF(KRET.EQ.0)  THEN
               IDUMMY=SYSTEM('[ -n "$jlogfile" ] && $DATA/postmsg'//
     .          ' "$jlogfile" "***WARNING: QUIPS FLAG FILE DOESN''T '//
     .          'CONTAIN ANY TIME-RELEVANT RECORD ENTRIES"')
            ENDIF
         ENDIF
         GOTO 700
      ELSE
...
C  -------------------------------------------------------------------
     ...
     .    M,CARDS(M)
         ...
         REWIND(91)
         IDUMMY=SYSTEM('[ -n "$jlogfile" ] && $DATA/postmsg'//
     .    ' "$jlogfile" "***WARNING: CORRUPT RECORD IN QUIPS FILE: '//
     .    '`cat ./fort.91`"')
         SKIPIT(M) = .TRUE.
...
C  ------------------------------------------------------------------
     ...
     .                     ABS(TAB3-ELAT(M)).LE.DEXY) THEN
      ...
      ENDDO LOOP2
      IDUMMY=SYSTEM('[ -f ./fort.91 ] && rm ./fort.91')
...
C  ------------------------------------------------------------------
  ...
  800 CONTINUE
     ...
     . ' TEST"/)', M,CARD
      ...
      REWIND(91)
      IDUMMY=SYSTEM('[ -n "$jlogfile" ] && $DATA/postmsg "$jlogfile" '//
     . '"***WARNING: CORRUPT RECORD IN QUIPS FILE: cat ./fort.91`"')
