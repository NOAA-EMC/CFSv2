...
C  -------------------------------------------------------------------
     ...
     .       ' "$jlogfile" "***WARNING: CORRUPT RECORD IN SDMEDIT '//
     .       'FILE: `cat ./fort.91`"')
...
C   ENTRY!)
     ...
     .             "-------------------")', M
                  ...
                  REWIND(91)
                  CALL SYSTEM('[ -n "$jlogfile" ] && '//
     .             '$DATA/postmsg "$jlogfile" "***WARNING: TOO '//
     .             'MANY WILDCARDS IN RECORD IN SDMEDIT FILE: '//
     .             '`cat ./fort.91`"')
                  SKIPIT(M) = .TRUE.
...
904   CONTINUE
      ...
      CALL SYSTEM('[ -n "$jlogfile" ] && $DATA/postmsg "$jlogfile" '//
     . '"**UNABLE TO ALLOCATE ARRAYS - ABNORMAL EXIT"')
      CALL W3TAGE('BUFR_EDTBFR')
      CALL ERREXIT(99)
905   CONTINUE
      PRINT *, '#####UNABLE TO DEALLOCATE ARRAYS - STOP 99'
      CALL SYSTEM('[ -n "$jlogfile" ] && $DATA/postmsg "$jlogfile" '//
     . '"**UNABLE TO DEALLOCATE ARRAYS - ABNORMAL EXIT"')
      CALL W3TAGE('BUFR_EDTBFR')
      CALL ERREXIT(99)
