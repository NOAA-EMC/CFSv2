...
C   RADIANCES ARE AVAILABLE VIA ORBITAL DOWNLOAD FROM THE SPACECRAFT,
C   AS WELL AS FROM THE REGIONAL ATOVS RETRANSMISSION SERVICES (RARS)
C   VIA DIRECT TRANSMISSION OF DATA TO A NETWORK OF GROUND STATIONS (IN
C   NEAR REAL-TIME). FOR ALL TYPES EXCEPT SSM/I, THE ALGORITHM SORTS
C   THE REPORTS IN ASCENDING ORDER OF LAT, LON AND OBS TIME (DAY DOWN
C   TO SECOND); AND IN THE DUPLICATE CHECKING, REPORTS ARE CHECKED FOR
C   LAT, LON AND OBS TIME (DAY DOWN TO SECOND) ALL INSIDE THE TOLERANCE
C   LIMITS.  FOR SSM/I, LAT IS REPLACED BY ORBIT NUMBER AND LON IS
C   REPLACED BY SCAN NUMBER IN BOTH THE SORTING AND DUPLICATE CHECKING
C   STEPS (LONGITUDE IS STILL CORRECTED TO BUFR STANDARD IF NECESSARY).
...
C   TIME WINDOW TRIMMING IS PERFORMED.  THE GEOGRAPHICAL FILTERING
C   PARAMETERS (EITHER A LAT/LON BOUNDARY, A CENTER POINT LAT/LON AND
C   RADIUS OR A 0.5 DEGREE GLOBAL LAT/LON GRID POINT MASK FILE) ARE
C   READ IN FROM THE SCRIPT ENVIRONMENT VARIABLE "LALO" VIA THE SYSTEM
C   LIBRARY ROUTINE "GETENV" (IF "LALO" IS MISSING OR "0", GEOGRAPHICAL
C   FILTERING IS NOT PERFORMED).  ALL FILE CONNECTIONS (EXCEPT STANDARD
...
ccccc call date_and_time(date,time); print*,time,' tabbing'
 ...
 1858 FORMAT(/'##WARNING: THE FIRST 25 REPORTS IN INPUT FILE HAVE A ',
     ...
     $             ' INPUT FILE HAVE A MISSING LAT, ALL REPORTS MAY '//
 ...
 1859 FORMAT(/'##WARNING: ALL REPORTS IN INPUT FILE HAVE MISSING LAT/',
     $ 'LON'/)
                  CALL SYSTEM('[ -n "$jlogfile" ] && $DATA/postmsg'//
     $             ' "$jlogfile" "***WARNING: ALL REPORTS IN INPUT '//
     $             'FILE HAVE MISSING LAT/LON, TYPE="'//SUBSET)
               ENDIF
               EXIT
            ENDIF
...
C  ------------------------------------
      ...
      IF(SUBSET(1:5).EQ.'NC005')  THEN
         CALL SATWND_ID(LUBFI,LUBFJ,JDUP,NDUP,JLON,NLON,LALO_8,
     $                  LATLON_TYPE,ICOMP) ! Satellite winds
...
C   OUTPUT FILE FOR ALL SATELLITE-DERIVED WIND TYPES.
C
C PROGRAM HISTORY LOG:
