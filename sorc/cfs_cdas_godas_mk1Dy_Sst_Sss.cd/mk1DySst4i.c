/*  reads GRIB files of HOURLY SST and creates a file for 1 day
    to be used by MOM4.  It assumes files are every 6 hours.
    input naming is assumed to have the date embedded: *yyyymmddhh*
    at the time of this writing (11-feb-2008), true diurnal SST
    files were not available.  if true diurnal files become
    available some modification may be needed (e.g. if hours are
    not 00, 06, 12, 18).                                         */
...
int year, year0;
YMD *tm;
float hr0 = 0.75, dh = 0.25, tk0 = 273.15;
...
char **argv;
{
  ...
  for (n = 0; n < nm2; n++) {
    if (n == 0) {
      sprintf(str,"ls %s/*.%4.4d%2.2d%2.2d%2.2d* > fname",path,
                                  (tm+1)->year,(tm+1)->month,(tm+1)->day,(tm+1)->hour);
    } else if (n >= nm2-2) {
      sprintf(str,"ls %s/*.%4.4d%2.2d%2.2d%2.2d* > fname",path,
                                  (tm+nm2-3)->year,(tm+nm2-3)->month,(tm+nm2-3)->day,(tm+nm2-3)->hour);
    } else {
      sprintf(str,"ls %s/*.%4.4d%2.2d%2.2d%2.2d* > fname",path,
...
*/
    sprintf(str,"$WGRIB -s %s | grep TMP | $WGRIB -i -s -bin -nh %s", inFile, inFile);
    system(str);
