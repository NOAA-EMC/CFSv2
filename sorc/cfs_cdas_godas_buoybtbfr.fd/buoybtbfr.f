...
!   PRGMMR: Diane C. Stokes  ORG: NP23        DATE: 2003-06-02
!
...
!     UNIQUE:    - none
!     LIBRARY:
!       W3LIB    - w3fs26, iw3jdn, w3tagb, w3tage, errexit
!        BUFR    - openbf, datelen, ufbint, closbf
!
!   SUBPROGRAMS CALLED FROM MAIN: (LIST ALL CALLED FROM MAIN)
!     UNIQUE:    - none
!     LIBRARY:
!       W3LIB    - w3fs26, iw3jdn, w3tagb, w3tage, errexit
!        BUFR    - openbf, ufbint, closebf
!
!   EXIT STATES:
...
c  output with modsbufr table (commented out)
      ...
      real(8) vec1(10),vsub(2,255),dval,wtmp,veco(8),vsubo(2,255)
      real(8) xtemp
...
*     data sid/'NW'/,ukey/'R'/,qkey/' '/
      ...
      call w3tagb('GODAS_BUOYBTBFR',2003,0164,0164,'NP23')
...
c read in buoy ids to reject
      ...
      if(ios.eq.0)then
        ...
        enddo
   90   continue
        print*,nrejid,' BUOYS ON REJECT LIST'
        if(nrejid.gt.0)then
...
*dcs* call openbf(lubfw,'OUT',lubfmt)
      ...
      DO WHILE(IREADNS(LUBFR,SUBSET,IDATE).EQ.0)
        if (subset.ne.'DBUOY   '.and.subset.ne.'NC001002')cycle
        ict = ict + 1
        lbad=.false.
...
*       icen = (iyr + 99)/100  !  century  1999->20, 2000->20, 2001->21
        ...
        if(kdate.ne.idate)then
          print*, 'ERROR?  kdate ne idate:',kdate, idate
...
c  TAO and TRITON ids exhausted.  
          ...
          print*,'dtyp set to BU for id and loc: ',cstnid,clat,clon
          dtyp='BU'
        else
          dtyp='BU'
        endif
