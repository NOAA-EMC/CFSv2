...
!</NAMELIST>  
  ...
  use mpp_mod, only : mpp_npes, stdout, stdlog, mpp_error, FATAL, &
              mpp_pe, mpp_declare_pelist,mpp_get_current_pelist,&
              mpp_set_current_pelist, mpp_root_pe,mpp_sum, mpp_sync
  ...
  use constants_mod, only : radius, epsln, rho_cp
!  use dr_oi_mod, only : dr_oi_init, dr_oi
...
! <IN NAME="Ext_mode" TYPE="ocean_external_mode_type"></IN>
  ...
  integer :: stdoutunit 
    ...
    stdoutunit=stdout()  !STEVE: moved from before to after the namelist statement upon suggestion of russ:
                         !STEVE: https://fms.gfdl.noaa.gov/pipermail/oar.gfdl.mom4p1/20100720/000474.html
