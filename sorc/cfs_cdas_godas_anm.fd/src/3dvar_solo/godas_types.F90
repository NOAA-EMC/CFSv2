...
!################################################################################################
  ...
  type, public :: ocean_clim_rstr_tracer_type
     character(len=32)  :: name
     character(len=32)  :: units
     character(len=128) :: longname
     ...
     real, dimension(isd:ied,jsd:jed,nk) :: frstr       ! restoration for tracer field
     ...
     character(len=128)          :: file_out            ! name for output restart file
     character(len=32)           :: name_in             ! name of variable in the restart file
  end type ocean_clim_rstr_tracer_type
...
! not STATIC_MEMORY
  ...
  type, public :: ocean_clim_rstr_tracer_type
     ...
     real(kind=4)                :: min_range           ! min value for diagnostic manager
     real(kind=4)                :: max_range           ! max value for to diagnostic manager
     logical                     :: init                ! true -> start is initial condition file
     character(len=128)          :: file_in             ! name for input restart file
     character(len=128)          :: file_out            ! name for output restart file
     character(len=32)           :: name_in             ! name of variable in the restart file
