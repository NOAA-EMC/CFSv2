...
!
!   restore_clim     = have GODAS do restoration to full T and S climate fields
!   num_c_rstr_tracers = number of tracers to be restored (def:0, T, S)
!   rtdata           = temperature climatology used for restoration
!   rsdata           = salinity climatology used for restoration
!   tz_c_damp        = damping factor for T (0: no damping, 1: set to restore field)
!   sz_c_damp        = damping factor for S (0: no damping, 1: set to restore field)
!
...
!   ext_wghts        = if .true. read lagrange smoother normalization weights from external file
...
logical :: single_incr, assrestrt, asm_ts_seq, asm_sfc_split, save_all_inv, godas_at_end, ext_wghts, debug_godas
...
integer :: num_c_rstr_tracers
real    :: tz_c_damp, sz_c_damp
real, allocatable, dimension(:,:,:) :: rtdata
real, allocatable, dimension(:,:,:) :: rsdata
logical :: restore_clim
...
integer, allocatable, dimension(:) :: id_c_rstr
...
!     wet_a is a mask set to 1 where assimilation is allowed and to 0 elsewhere,
!     wet_a is allocated as (i,j,k), wgns, s1, s2, dpth are allocated as (i,j)
!
real, save, dimension(:,:), allocatable :: wcn, wea, wwe, wno, wso
real, save, dimension(:,:), allocatable :: wcn_s, wea_s, wwe_s, wno_s, wso_s
real, save, dimension(:,:), allocatable :: wgns, s1, s2, dpth, wet_1
real, save, dimension(:,:,:), allocatable :: wet_a
!
