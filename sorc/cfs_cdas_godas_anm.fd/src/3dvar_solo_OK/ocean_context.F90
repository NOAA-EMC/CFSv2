...
!   call mpp_open(unit, trim(T_prog(1)%file_in), action=MPP_RDONLY, form=MPP_NETCDF, threading=MPP_MULTI, fileset=MPP_SINGLE)
    ...
    do k=1,nk
      do j=jsd,jed
        ...
        do i=isd,ied
          ...
          if (ii .gt. ni) ii=ni
          if (ii .lt. 1) ii=1
          T_prog(1)%field(i,j,k,taup1) = temp(ii,jj,k)
          T_prog(2)%field(i,j,k,taup1) = salt(ii,jj,k)
        enddo
      enddo
    enddo
