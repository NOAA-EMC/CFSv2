...
! use godas_data_mod,             only: godas_at_end
!
implicit none
...
!    do n=1,num_c_rstr_tracers
!      range_array(1) = T_c_rstr(n)%min_range
!      range_array(2) = T_c_rstr(n)%max_range
!      id_c_rstr(n) = register_diag_field ('ocean_model', trim(T_c_rstr(n)%name), &
!           Grd%tracer_axes(1:3),                                             &
!           Time%model_time, trim(T_c_rstr(n)%longname), trim(T_c_rstr(n)%units), &
...
!   do n=1,num_c_rstr_tracers
!     if (.not. T_c_rstr(n)%init) then
!       write (stdout(),'(/a,a)') 'Expecting to read a GODAS restart file, ', T_c_rstr(n)%file_in
!     endif

!     T_c_rstr(n)%frstr(:,:,:) = 0.0

!     if (file_exist(T_c_rstr(n)%file_in)) then
!       call read_data(T_c_rstr(n)%file_in, T_c_rstr(n)%name_in, T_c_rstr(n)%frstr(:,:,:), Dom%domain2d, timelevel=1)
