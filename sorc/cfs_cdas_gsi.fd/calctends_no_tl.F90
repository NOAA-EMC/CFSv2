...
! Declare local variables
  real(r_kind),dimension(lat2,lon2,nsig+ione):: pri
  ...
  real(r_kind),dimension(lat2,lon2,nsig+ione):: p3d_x,p3d_y
...
!!!$omp parallel private(nth,tid,i,j,k,tmp,tmp2, &
...
! 3) load actual dp/dt
  do j=jstart,jstop
     do i=1,lat2
        p_t(i,j)=prsth(i,j,1)-what(i,j,1)
     end do
  end do
