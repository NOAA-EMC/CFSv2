...
subroutine sigvar(sighead,cvar)
  ...
  case('IHR','ihr')
    ...
    print '(a)',cval(1:lval)
  case('VDATE','vdate')
    call w3movdat((/0.,sighead%fhour,0.,0.,0./),&
  ...
  case('VCOORD','vcoord')
    ...
    enddo
  case('CP','cp')
    if (sighead%idvm == 32) then
      print '(5g16.8)',sighead%cpi(1:sighead%ntrac+1)
    else
      print *,' no cp available',' idvm=',sighead%idvm
    endif
  case('RGAS','rgas')
    if (sighead%idvm == 32) then
      print '(5g16.8)',sighead%ri(1:sighead%ntrac+1)
    else
      print *,' no rgas available',' idvm=',sighead%idvm
    endif
  case('?')
    print '(a)','Choose from:'
    print '(a)','  filetype'
