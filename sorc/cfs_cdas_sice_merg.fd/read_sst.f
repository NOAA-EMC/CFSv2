...
!    2007-1031   S. Moorthi - generalized
      ...
      real (kind=4), allocatable :: sst(:,:)
      real fland
!
      character(len=120) :: sstgrib,sstnc
!
...
!
      ...
      sstgrib='sstgrb.gdas.'//charymdh
      call baopenr(nf,sstgrib,ierr)
!
! SST: JPDS(5)=11 JPDS(6)=1 JPDS(7)=0
!
      me      = 0
      lskip   = -1
      JPDS(:) = -1
...
!
      ...
      allocate (fi(im,jm), sst(im,jm), xlon(im), ylat(jm), cosl(jm), wgt(jm))
!
...
!     call FillLand(sst,im,jm,fland)

      fi=sst
      sstnc='sst.nc'

      call sst2nc(sstnc,im,jm,xlon,ylat,fi)
