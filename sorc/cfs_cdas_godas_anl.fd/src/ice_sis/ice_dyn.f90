...
contains
  ...
  subroutine ice_dynamics(ci, hs, hi, ui, vi, sig11, sig22, sig12, uo, vo,       &
    ...
    do j = jsc, jec
       do i = isc, iec
          if(wetv(i,j) .and. miv(i,j) > MIV_MIN ) then ! values for velocity calculation (on v-grid)
    ...
    do l=1,evp_sub_steps
       ...
       call mpp_update_domains(ui, Domain)
       call mpp_update_domains(vi, Domain)
       !
       ...
       do j = jsc, jec
          do i = isc, iec
             if( wett(i,j).and.(ci(i,j)*(DI*hi(i,j)+DS*hs(i,j))>MIV_MIN) ) then
       ...
       call mpp_update_domains(sig11, Domain)
       call mpp_update_domains(sig22, Domain)
       call mpp_update_domains(sig12, Domain)
       ...
       do j = jsc, jec
          do i = isc, iec
             if( wetv(i,j).and.(miv(i,j)>MIV_MIN)) then ! timestep ice velocity (H&D eqn 22)
    ...
    do j = jsc, jec
       do i = isc, iec
          if( wetv(i,j) .and. miv(i,j)>MIV_MIN ) then
