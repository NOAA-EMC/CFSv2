...
contains
  ...
  subroutine uv_to_t(uv, t)
    real,    intent(in   ), dimension(isd:ied,jsd:jed) :: uv
    real,    intent(  out), dimension(isc:iec,jsc:jec) :: t
  ...
  !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~!
  function dTdx(T)
    real, intent(in   ), dimension(isd:ied,jsd:jed) :: T 
  ...
  subroutine t_to_uv(t, uv)
    real,    intent(in ), dimension(isd:ied,jsd:jed) :: t
    real,    intent(out), dimension(isc:iec,jsc:jec) :: uv
  ...
  subroutine vel_t_to_uv(tx, ty, uvx, uvy)
    real,    intent(in   ), dimension(isd:ied,jsd:jed) :: tx, ty
    real,    intent(  out), dimension(isc:iec,jsc:jec) :: uvx, uvy
  ...
  subroutine set_ice_grid(ice_domain, dt_slow, dyn_sub_steps_in, &
    ...
    rcode = nf_open('INPUT/grid_spec.nc',NF_SHARE,ncid)
    if (rcode/=0) call error_mesg ('ice_model_mod', 'cannot open INPUT/grid_spec.nc', FATAL)
    ...
    start = 1; nread = 1;
    start(1) = isc; nread(1) = iec-isc+1;
    start(2) = jsc; nread(2) = jec-jsc+1;
    ...
    start = 1; nread = 1;
    rcode = nf_inq_varid(ncid, 'x_vert_T', varid)
    if (rcode/=0) then
       rcode = nf_inq_varid(ncid, 'geolon_vert_t', varid)
       if (rcode/=0) call error_mesg ('ice_model_mod', &
            'cannot find x_vert_t or geolon_vert_t on INPUT/grid_spec.nc', FATAL)
       nread(1) = im+1; nread(2) = jm+1;
       rcode = nf_get_vara_double(ncid, varid, start, nread, geo_lonv)
    else
       allocate ( x_vert_t(im,jm,4) )
       nread(1) = im; nread(2) = jm; nread(3) = 4
       rcode = nf_get_vara_double(ncid, varid, start, nread, x_vert_t)
    ...
    start = 1; nread = 1;
    rcode = nf_inq_varid(ncid, 'y_vert_T', varid)
    if (rcode/=0) then
       rcode = nf_inq_varid(ncid, 'geolat_vert_t', varid)
       if (rcode/=0) call error_mesg ('ice_model_mod', &
            'cannot find y_vert_t or geolat_vert_t on INPUT/grid_spec.nc', FATAL)
       nread(1) = im+1; nread(2) = jm+1;
       rcode = nf_get_vara_double(ncid, varid, start, nread, geo_latv)
    else
       allocate ( y_vert_t(im,jm,4) )
       nread(1) = im; nread(2) = jm; nread(3) = 4
       rcode = nf_get_vara_double(ncid, varid, start, nread, y_vert_t)
    ...
    if (rcode/=0) call error_mesg ('ice_model_mod', &
         'cannot find AREA_OCN on INPUT/grid_spec.nc', FATAL)
    rcode = nf_get_vara_double(ncid, varid, start, nread, cell_area)
...
!   consider arbitray layout
  ...
  subroutine ice_line(year, day, second, cn, sst)
    ...
    real, dimension(isc:iec,jsc:jec,1:km), intent(in) :: cn
    real, dimension(isc:iec,jsc:jec),      intent(in) :: sst
  ...
  subroutine ice_advect(ui, vi, trc, uf, vf)
    real, intent(in   ),           dimension(isd:ied,jsd:jed) :: ui, vi  ! advecting velocity
    real, intent(inout),           dimension(isd:ied,jsd:jed) :: trc     ! tracer to advect
    real, optional, intent(inout), dimension(isc:iec,jsc:jec) :: uf, vf
  ...
  subroutine cut_check(mesg, uv)
    ...
    real, intent(inout), dimension(isd:ied,jsd:jed) :: uv
