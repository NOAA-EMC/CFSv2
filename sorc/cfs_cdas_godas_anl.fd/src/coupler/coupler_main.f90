...
!-----------------------------------------------------------------------
  ...
  character(len=128) :: version = '$Id: coupler_main.f90,v 1.4 2004/12/10 19:35:16 gtn Exp $'
  character(len=128) :: tag = '$Name: mom4p0d $'
...
!----- write restart file ------
 ...
 if ( mpp_pe() .eq. mpp_root_pe() ) then
   write( unit, '(i4,i2.2,i2.2)') date(1), date(2), date(3)
 endif
 call mpp_close(unit)
