...
#include "netcdf.inc"
  ...
  !--- version information ---------------------------------------------
  character(len=128) :: version = '$Id: regrid.F90,v 1.4 2005/05/12 21:41:05 gtn Exp $'
...
!  real, dimension(max_flds)           :: missing_value =  -1e34
...
contains
  ...
  !--- on C, E, or N-cell, call horiz_interp_init to set up the interpolation.
  ...
  subroutine setup_interp
    ...
    if( any(fld_pos == 'C') ) then
       call horiz_interp_init(Interp_c, geolon_c*D2R, geolat_c*D2R, xb_dst(isc+1:iec+1)*D2R, &
    ...
    if( any(fld_pos == 'E') ) then
       call horiz_interp_init(Interp_e, geolon_e*D2R, geolat_e*D2R, xb_dst(isc+1:iec+1)*D2R, &
            yt_dst(jsc:jec)*D2R, interp_method="spherical",   &
            num_nbrs=num_nbrs, max_dist=max_dist, src_modulo = src_is_cyclic)
    endif

    if( any(fld_pos == 'N') ) then
       call horiz_interp_init(Interp_n, geolon_n*D2R, geolat_n*D2R, xt_dst(isc:iec)*D2R, &
            yb_dst(jsc+1:jec+1)*D2R, interp_method="spherical",   &
            num_nbrs=num_nbrs, max_dist=max_dist, src_modulo = src_is_cyclic)
    endif
