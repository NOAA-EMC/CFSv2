...
!
  ...
  use mpp_mod, only: mpp_init, mpp_pe, mpp_npes, mpp_root_pe, &
                     ...
                     mpp_send,mpp_recv,mpp_get_current_pelist      !-->cpl insertion add mppsend, mpp_recv --<
  use mpp_io_mod, only: mpp_open, mpp_close, &
                        MPP_NATIVE, MPP_RDONLY, MPP_DELETE
  use mpp_domains_mod, only: mpp_broadcast_domain, &
                             mpp_get_global_domain                 !-->cpl insertion mpp_get_global_domain
...
! ----- coupled model time -----
  ...
  type (time_type) :: Time, Time_init, Time_end, Time_step_ocean, &
                      Time_step_atmos, Time_step_cpld,Time_step_aocpl
  type(time_type) :: Time_atmos, Time_ocean
  integer :: num_ocean_calls, num_atmos_calls, num_aocpl, no, na
...
! </NAMELIST>
  ...
  integer :: dt_aocpl = 0  ! fluxes passed between atm & ocean
  ...
  logical :: coldstart_daily_flx=.FALSE.
...
!--> cpl insertion    cpl definition
  integer :: coupler_id=0, ocean_id=2, num_aflx, cpl_rank, ocean_mpi_comm,worldpeset
  logical :: no_sync=.true., lcplocn=.false.
  integer,dimension(256) :: pelist
  character(80) :: flnm
!--> cpl insertion
...
!--> cpl deletion
!--> cpl insertion
  call mpp_init(myid=ocean_id, cpl_id=coupler_id, cpl_rank=cpl_rank,ocean_comm=ocean_mpi_comm, &
       ocean_npes=ocean_npes,pelist=pelist)
  write( text,'(a,i4)' )'coupelr rank=', cpl_rank
  call mpp_error( NOTE, 'Ocean: after mpp_init: '//trim(text) )
!--> cpl insertion
...
!these clocks are on the global pelist
  ...
  call coupler_init
  call mpp_error( NOTE, 'Ocean: after coupler_init: ' )
...
!
   ...
   call mpp_error( NOTE, 'Ocean: in slow loop: '//trim(text) )
!
     if( Atm%pe )then
...
!   ------ atmos/fast-land/fast-ice integration loop -------
         ...
         do na = 1, num_atmos_calls
            ...
            num_aflx=(nc-1)*num_atmos_calls+na
            lcplocn=mod(num_aflx,num_aocpl).eq.0
            if (lcplocn) coldstart_daily_flx=.false.
        write( text,'(a,i4)' )'na=', na
        write( flnm,'(i3.3,a,i3.3,a,i3.3)' )nc,'_',na,'_',mpp_pe()
        call mpp_error( NOTE, 'Ocean: after sfc_boundary_layer: '//trim(text) )
            Time_atmos = Time_atmos + Time_step_atmos

            if (do_flux) then
              call sfc_boundary_layer( REAL(dt_atmos), Time_atmos, lcplocn, &
                                       flnm, &
...
!      ---- atmosphere down ----
            ...
            call flux_down_from_atmos( Time_atmos, lcplocn, Atm, Land, Ice, &
...
!      ---- ice model ----
            if (do_ice) &
              call update_ice_model_fast( lcplocn, Atmos_ice_boundary, Ice )
            
!      --------------------------------------------------------------
!      ---- atmosphere up ----

            call flux_up_to_atmos( Time_atmos, Land, Ice, Atm,Land_ice_atmos_boundary )
...
!   ------ slow-ice model ------
         ...
         if (do_ice) call update_ice_model_slow_dn(lcplocn, coldstart_daily_flx, &
                     Atmos_ice_boundary, Land_ice_boundary, Ice )
         Time = Time_atmos
     end if                     !Atm%pe block
...
!messages
!
    write( text,'(a,i6)' )'Coupler rank: ', Atm%coupler_rank
    call mpp_error( NOTE, 'coupler_init: '//trim(text) )
    write( text,'(a,2i6)' )'Ocean PE range: ', atmos_pe_start,atmos_pe_end
    call mpp_error( NOTE, 'coupler_init: '//trim(text) )
...
!--> cpl insertion
       if (pe.EQ.mpp_root_pe() )then
         if( .not.allocated(ibuf) ) allocate(ibuf(2))
         if( .not.allocated(buf) ) allocate(buf(2))
         if( allocated(ibuf) .and. size(ibuf).lt.2) then
           deallocate(ibuf)
           allocate(ibuf(2))
         endif
         ibuf(1)=ocean_id
         call mpp_set_current_pelist(no_sync=no_sync)
         ibuf(2)=mpp_root_pe()
         call mpp_set_current_pelist(pesetnum=worldpeset,no_sync=no_sync)
         call mpp_send(ibuf, 2, cpl_rank)   !suppose tag=1
         buf(1)=dt_atmos
         buf(2)=dt_aocpl
         if(buf(2) == 0 ) buf(2) = dt_ocean
         call mpp_send(buf, 2, cpl_rank)   !suppose tag=1
         call mpp_set_current_pelist(Atm%pelist,no_sync=no_sync)
         deallocate(ibuf)
       endif
!<-- cpl insertion
...
!----- compute the time steps ------
    ...
    Time_step_aocpl = set_time (dt_aocpl,0)
...
!----- determine maximum number of iterations per loop ------
    ...
    num_aocpl       = Time_step_aocpl / Time_step_atmos
    if (num_aocpl == 0) num_aocpl=num_atmos_calls
...
!------ grid info now comes from grid_spec file
    ...
    if( Atm%pe )then
        call mpp_set_current_pelist(Atm%pelist)
!---- atmosphere ----
        call atmos_model_init( Atm, Time_init, Time, Time_step_atmos ,ocean_mpi_comm)
