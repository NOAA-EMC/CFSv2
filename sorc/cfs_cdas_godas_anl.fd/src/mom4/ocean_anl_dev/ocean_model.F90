...
!
! Top level module for GODAS analysis.  Contains routines for 
! initialization, termination and update of ocean model state.
! This does analysis only, using restart as first guess and
! writing a new corrected restart.  No time-step of the dynamical
! model is done.
!
...
use ocean_freesurf_mod,           only: ocean_freesurf_init, ocean_freesurf_end
...
use godas_obs_mod,              only: godas_obsz_init, godas_obs0_init, godas_obsa_init, godas_obs_end
...
#include <ocean_memory.h>
  ...
  character(len=128) :: version = '$Id: ocean_model.F90,v 1.6 2004/12/10 19:35:16 gtn Exp $'
...
! RMV   call ocean_tpm_end(Thickness)
! RMV   call ocean_velocity_end(Time, Velocity, ens_ocean)
! RMV   call ocean_freesurf_end(Time, Ext_mode, ens_ocean)
! RMV   call ocean_density_end(Time, Dens, ens_ocean)
! RMV   if(have_obc) call ocean_obc_end(have_obc)
! RMV   call ocean_sfc_end(Ocean, ens_ocean)
#ifdef ENABLE_GDS
    call godas_obs_end(Time)
    call godas_end(Time, T_cor(:))
#endif
