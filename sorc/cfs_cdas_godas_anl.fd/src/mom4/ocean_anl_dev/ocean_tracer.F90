...
use ocean_neutral_physics_mod,  only: ocean_neutral_physics_end
...
character(len=256) :: version='CVS $Id: ocean_tracer.F90,v 1.9 2005/05/12 21:41:04 gtn Exp $'
character(len=256) :: tagname='Tag $Name: mom4p0d $'
...
! RMV    ! modifications due to partial bottom cell
! RMV    write (stdout(),*) 'After reading ic, linearly interpolate ',trim(T_prog(n)%name),' to partial cell bottom.'
! RMV    do j=jsc,jec
! RMV       do i=isc,iec
! RMV          kb = Grd%kmt(i,j)
! RMV          if (kb .gt. 1) then
! RMV              fact = Thickness%dhwt(i,j,kb-1)/Grd%dzw(kb-1)
! RMV              T_prog(n)%field(i,j,kb,tau) =                  &
! RMV                   T_prog(n)%field(i,j,kb-1,tau) -           &
! RMV                   fact*(T_prog(n)%field(i,j,kb-1,tau) -     &
! RMV                   T_prog(n)%field(i,j,kb,tau))
! RMV          endif

! RMV          ! zero out tracers in land points
! RMV          do k = kb+1,nk
! RMV             T_prog(n)%field(i,j,k,tau) = 0.0
! RMV          enddo
...
! RMV  do n=1,num_diag_tracers

! RMV    filename = T_diag(n)%file_out

! RMV    if (filename .ne. ' ') then  
! RMV      call write_data(filename, trim(T_diag(n)%name), T_diag(n)%field(:,:,:), domain = Dom%domain2d,&
! RMV                      append_pelist_name = ens_ocean)

! RMV      write(stdout(),*) 'Ending ',trim(T_diag(n)%name), ' chksum       ==>'
! RMV      call tracer_diag_chksum(Time, T_diag(n), chksum)
! RMV      if (mpp_pe() == mpp_root_pe()) then  
! RMV        write(unit,*) trim(T_diag(n)%name)//'       chksum =', chksum     
! RMV      endif 
...
! RMV  call ocean_neutral_physics_end(ens_ocean)
