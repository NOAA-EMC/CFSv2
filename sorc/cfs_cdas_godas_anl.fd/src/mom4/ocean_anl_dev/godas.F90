...
use mpp_mod,           only: mpp_sum, mpp_sync, ALL_PES
...
use godas_data_mod,             only: num_cor_tracers, num_rstr_tracers
use godas_data_mod,             only: sst_damp, sss_damp
use godas_data_mod,             only: kass, kass2, nsgobs, nsgsobs, maxits, npits, jemx
use godas_data_mod,             only: no_asm_rep, asm_cnt
use godas_data_mod,             only: gds_step, acoef, hrscl, vcvn, vsclf, no_lat_mx
...
use godas_data_mod,             only: id_dg_cor, id_dg_rstr
use godas_data_mod,             only: g_cg, d_cg, f_cg, e_cg, t_cg, h_cg
use godas_data_mod,             only: gds_freq, alrm_dur, ovr_alrm
use godas_data_mod,             only: assrestrt, rstrestrt, debug_godas, tuningF
...
namelist /godas_nml/ num_cor_tracers, kass, nsgobs, nsgsobs, maxits, npits, &
                     gds_step, no_asm_rep, acoef, hrscl, vcvn, vsclf, no_lat_mx, &
                     tz_wndw_fwd, tz_wndw_bwd, sz_wndw_fwd, sz_wndw_bwd, &
                     t0_wndw_fwd, t0_wndw_bwd, s0_wndw_fwd, s0_wndw_bwd, &
                     al_wndw_fwd, al_wndw_bwd, assrestrt,  &
                     num_rstr_tracers, sst_damp, sss_damp, rstrestrt, &
                     tuningF, debug_godas
...
function godas_init (Grid, Domain, Time, T_prog, num_cor, debug) &
  ...
  integer               :: pe, pes, npes, ierr, mtss, mtsd
  integer               :: year, month, day, hour, minute, second
  integer               :: i, j, k, ii, jj, ig, jg, kg
  ...
  real(kind=4), dimension(720,410)   :: bufk
! real(kind=4), dimension(:,:), allocatable   :: bufk
...
! provide for namelist over-ride
  ...
  allocate( id_dg_cor (num_cor_tracers) )
...
!   allocate (bufk(isg:ieg,jsg:jeg,kass))
    do nf=1,ntable
...
! DBG
      do pes=0,npes-1
      if (pe .eq. pes) then
!       allocate (bufk(720,410))
        nu=1000+pe
        open(unit=nu,file=trim(data_table(nf)%file_name),status='OLD', &
...
! RMV  do n=1,num_cor_tracers
! RMV    range_array(1) = T_cor(n)%min_range
! RMV    range_array(2) = T_cor(n)%max_range
! RMV    id_dg_cor(n) = register_diag_field ('ocean_model', trim(T_cor(n)%name), &
! RMV         Grd%tracer_axes(1:3),                                             &
! RMV         Time%model_time, trim(T_cor(n)%longname), trim(T_cor(n)%units), &
! RMV         missing_value=missing_value, range=range_array)
! RMV  enddo
...
! RMV  do n=1,num_cor_tracers
! RMV    if (id_dg_cor(n) > 0) used = send_data (id_dg_cor(n), T_cor(n)%fcor(:,:,:), &
! RMV                                Time%model_time,rmask=Grd%tmask(:,:,:), &
! RMV                                is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
! RMV  enddo

! send restorations to diag_manager

! RMV  do n=1,num_rstr_tracers
! RMV    if (id_dg_rstr(n) > 0) used = send_data (id_dg_rstr(n), T_rstr(n)%frstr(:,:), &
! RMV                                Time%model_time,rmask=Grd%tmask(:,:,1), &
! RMV                                is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
! RMV  enddo
...
!-----------------------------------------------------------------------
  ...
  do k = 1,kass
    do j = jsc,jec
      do i = isc,iec
        gh_new = gh_new + g_cg(i,j,k)*h_cg(i,j,k)*Grd%tmask(i,j,k)
        gh_new = gh_new + g_cg(i,j,k+kass)*h_cg(i,j,k+kass)*Grd%tmask(i,j,k)
        df_val = df_val + d_cg(i,j,k)*f_cg(i,j,k)*Grd%tmask(i,j,k)
        df_val = df_val + d_cg(i,j,k+kass)*f_cg(i,j,k+kass)*Grd%tmask(i,j,k)
      enddo
    enddo
  enddo
  call mpp_sum(gh_new)
