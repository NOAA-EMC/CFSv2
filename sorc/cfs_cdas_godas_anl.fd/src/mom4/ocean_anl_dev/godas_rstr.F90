...
use godas_data_mod,             only: num_rstr_tracers, id_dg_rstr
...
function godas_rstr_init (Grid, Domain, Time, T_prog) result (T_rstr)
  ...
  allocate( id_dg_rstr (num_prog_tracers) )
...
!
  do n=1,num_rstr_tracers
    if (n == index_temp) then
      ...
      T_rstr(n)%init=.false.
      T_rstr(n)%file_in='ocean_rstr.res.nc'
      T_rstr(n)%file_out='ocean_rstr.res.nc'
  ...
  do n=1,num_rstr_tracers
    ...
    id_dg_rstr(n) = register_diag_field ('ocean_model', trim(T_rstr(n)%name), &
         Grd%tracer_axes(1:2),                                             &
