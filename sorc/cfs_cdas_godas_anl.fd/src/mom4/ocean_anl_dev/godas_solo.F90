program main
!  
! Driver for single ocean-only 3DVAR analysis.
!
 ...
 use time_manager_mod,         only: print_date, print_time
 ...
 integer :: pe
  ...
  namelist /godas_solo_nml/ date_init, calendar, months, days, hours, minutes, seconds, dt_cpld, dt_ocean
...
! initialize shared modules
  ...
  write (stdout(), godas_solo_nml)  
  write (stdlog(), godas_solo_nml)
  ...
  case default
     call mpp_error( FATAL, 'godas_solo: godas_solo_nml entry calendar must be one of JULIAN|NOLEAP|THIRTY_DAY|NO_CALENDAR.' )
  end select 

  ! get godas_solo restart : this can override settings from namelist
  if (file_exist('INPUT/godas_solo.res')) then
      call mpp_open(unit,'INPUT/godas_solo.res',form=MPP_ASCII,action=MPP_RDONLY)
      read(unit,*) calendar_type 
      read(unit,*) date_init
      read(unit,*) date
      call close_file(unit)
  endif
  ! may be used with a coupler restart, so
  if (file_exist('INPUT/coupler.res')) then
      call mpp_open(unit,'INPUT/coupler.res',form=MPP_ASCII,action=MPP_RDONLY)
      read(unit,*) calendar_type
      read(unit,*) date_init
      read(unit,*) date
      call close_file(unit)
  ...
  if (sum(date_init) <= 0) then
      call mpp_error(FATAL,'==>Error from godas_solo_mod: date_init must be set either in godas_solo.res or in godas_solo_nml')
  else
      Time_init  = set_date(date_init(1),date_init(2), date_init(3), &
           date_init(4),date_init(5),date_init(6))
  endif

  if (file_exist('INPUT/godas_solo.res') .or. file_exist('INPUT/coupler.res')) then
 ...
 pe = mpp_pe()
 if (pe .eq. 0) then
 write(6,'(a,i4,i6)') 'GODAS_T',pe,days
 call print_date(Time_start,'GODAS_T start',6)
 call print_date(Time_end,'GODAS_T end',6)
 call print_time(Run_len,'GODAS_T len',6)
 call print_time(Time_step_coupled,'GODAS_T ts_c',6)
 call print_time(Time_step_ocean,'GODAS_T ts_o',6)
 write(6,'(a,i6)') 'GODAS_T nc', num_cpld_calls
 write(6,'(a,i6)') 'GODAS_T no', num_ocean_calls
 endif
...
! RMV  call data_override_init(Ocean_domain_in = Ocean_sfc%domain)

! RMV  override_clock = mpp_clock_id('Override', flags=flags,grain=CLOCK_COMPONENT)
...
! RMV
 ...
 if (pe .eq. 0) then
 call print_date(Time,'GODAS_T t=',6)
 endif
! RMV  do nc=1, num_cpld_calls
     
! RMV     call mpp_clock_begin(override_clock)
! RMV     call ice_ocn_bnd_from_data(Ice_ocean_boundary)
! RMV     call mpp_clock_end(override_clock)
     
     ! loop over the ocean calls 
! RMV     do no=1, num_ocean_calls
! RMV        ocean_seg_start = ( no .eq. 1 )     
! RMV        ocean_seg_end   = ( no .eq. num_ocean_calls )
        ocean_seg_start = .true.
        ocean_seg_end   = .true.
        call update_ocean_model(Ice_ocean_boundary, Ocean_sfc, ocean_seg_start, ocean_seg_end, num_ocean_calls)
! RMV        Time = Time + Time_step_ocean
! RMV     enddo
 if (pe .eq. 0) then
 call print_date(Time,'GODAS_T t=',6)
 endif
...
! RMV  enddo
  ...
  call mpp_set_current_pelist() 
    ...
    ! write restart file
! RMV    call mpp_open( unit, 'RESTART/godas_solo.res', nohdrs=.TRUE. )
! RMV    if ( mpp_pe().EQ.mpp_root_pe() )then
! RMV        write( unit, '(i6,8x,a)' )calendar_type, &
! RMV             '(Calendar: no_calendar=0, thirty_day_months=1, julian=2, gregorian=3, noleap=4)'

! RMV        call get_date(Time_init,yr,mon,day,hr,min,sec)
! RMV        write( unit, '(6i6,8x,a)' )yr,mon,day,hr,min,sec, &
! RMV             'Model start time:   year, month, day, hour, minute, second'
! RMV        call get_date(Time_end ,yr,mon,day,hr,min,sec)
! RMV        write( unit, '(6i6,8x,a)' )yr,mon,day,hr,min,sec, &
! RMV             'Current model time: year, month, day, hour, minute, second'
! RMV    end if
! RMV    call mpp_close(unit)

 ! write an end date tag file
 call mpp_open( unit, 'end_date', nohdrs=.TRUE. )
 if ( mpp_pe() .eq. mpp_root_pe() ) then
   call get_date(Time_end ,yr,mon,day,hr,min,sec)
   write( unit, '(i4,i2.2,i2.2)') yr, mon, day
 endif
 call mpp_close(unit)
