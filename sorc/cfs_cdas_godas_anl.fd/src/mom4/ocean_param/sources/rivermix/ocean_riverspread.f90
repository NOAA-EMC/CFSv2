...
!
!<OVERVIEW>
! Spread runoff over more than one river-mouth point. 
!</OVERVIEW>
!
!<DESCRIPTION>
! At some coastal ocean gridpoints, the river flux contribution to 
! (p-e+r) may be very large because of river runoff. Therefore, spread the
! large river runoff over the neighboring pairs of gridpoints.
! Annual mean river runoff greater than 0.05 m/day is considered to be
! very large.
!</DESCRIPTION>
!
! <INFO>
...
! </INFO>
!
use field_manager_mod,   only: MODEL_OCEAN, find_field_index, get_field_methods, method_type, get_field_info, parse
use fms_mod,             only: write_version_number, open_namelist_file, check_nml_error, close_file
use fms_mod,             only: stdout, stdlog, FATAL, WARNING
use mpp_domains_mod,     only: domain2D, mpp_update_domains, mpp_define_domains, cyclic_global_domain, global_data_domain
...
character(len=256) :: version=&
       '$Id: ocean_riverspread.f90,v 1.5 2005/05/12 21:41:05 gtn Exp $)'
character (len=128) :: tagname = &
     '$Name: mom4p0d $'

integer :: isd, ied, jsd, jed, isc, iec, jsc, jec, nk

logical :: debug_riverspread     = .false.
logical :: module_is_initialized = .FALSE.

namelist /ocean_riverspread_nml/ debug_riverspread 
...
!  <DATA NAME="debug_riverspread" TYPE="logical">
!  For debugging 
!  </DATA> 
!</NAMELIST>

subroutine ocean_riverspread_init(Grid, Domain)
  ...
  if ( module_is_initialized ) then  
    call mpp_error(FATAL, '==>Error in ocean_riverspread_mod (ocean_riverspread_init): module already initialized')
  endif 
  ...
  n = find_field_index(MODEL_OCEAN,'spread_horz')
  ...
  write (stdlog(),ocean_riverspread_nml)
  write (stdout(),'(/)') 
  write (stdout(),ocean_riverspread_nml)
  ...
  if(nspread == 0) then 
     call mpp_error(WARNING,'==>Warning in ocean_riverspread_mod: No river spreading points chosen although riverspread=.true.')
     return
  endif
  if(Grd%tripolar) then 
    call mpp_error(WARNING,'==>Warning in ocean_riverspread_mod: river spreading not implemented for points across bipolar fold.')
  endif
  ...
  if(spread_halo > Dom%xhalo .or. spread_halo > Dom%yhalo ) then 
    write(stdout(),*)'Defining extra tracer arrays for reiver "spread_domain". Halo for spreading = ', spread_halo
    write(stdout(),*)'Halos for the spreading regions = ', halon
  endif 

  ! define arrays and spread_domain
  if(spread_halo <=Dom%xhalo .and. spread_halo <= Dom%yhalo) then 

     write(stdout(),*)'Local computational domain is large enough for chosen points in river spreading,'
  ...
  if(spread_halo > Dom%xhalo .or. spread_halo > Dom%yhalo) then 
     write(stdout(),*)'The model local computational domain has a halo = ',Dom%xhalo, Dom%yhalo
     write(stdout(),*)'This is smaller than the halo required for river spreading.'
     write(stdout(),*)'For spreading, will define a new domain type with halo = ', spread_halo

     call mpp_define_domains((/1,Grid%ni,1,Grid%nj/),Domain%layout,Spread_domain%domain2d, &
  ...
  do nsp=1,nspread
    if(error_spread(nsp)) then
      call mpp_error(FATAL,'==>Error in ocean_riverspread_mod: problem in river spread initialization.')
    endif
  enddo
