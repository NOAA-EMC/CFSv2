...
!  <DATA NAME="polar_filter_on" TYPE="logical">
...
!
  ...
  use constants_mod,    only: radius, radian, epsln, tfreeze
...
! for diagnostics 
  ...
  character(len=128) :: version=&
       '$Id: ocean_polar_filter.f90,v 1.6 2005/05/12 21:41:05 gtn Exp $'
  ...
  logical :: polar_filter_on       = .false.
  logical :: polar_filter_field    = .true.
  logical :: polar_filter_tendency = .false.
  logical :: module_is_initialized = .FALSE.

  namelist /ocean_polar_filter_nml/ polar_filter_on, polar_filter_tendency, polar_filter_field, &
...
! </OVERVIEW>
!
  subroutine ocean_polar_filter_init(Grid, Domain, Time, T_prog, d_time, itemp)
    ...
    if ( module_is_initialized ) then 
      call mpp_error(FATAL, '==>Error from ocean_polar_filter_mod (ocean_polar_filter_init): module already initialized')
    endif 
    ...
    write (stdout(),'(/)')
    write (stdout(),ocean_polar_filter_nml)  
    write (stdlog(),ocean_polar_filter_nml)
    ierr = check_nml_error(io_status,'ocean_polar_filter_nml')
    call close_file (ioun)

    if(polar_filter_on) then 
      ...
      call mpp_error(NOTE, '==>NOTE from ocean_polar_filter_mod: NOT using ocean_polar_filter_mod.')
      return 
    endif 
    ...
    write (stdout(),'(a)') 'Polar filtering details for latitudes'
...
!   set up model indices for filtering polar latitudes
    ...
    do j=1,Grd%nj
       if ((j .le. jft1 .or. j .ge. jft2) .and. j .ge. jfrst) then
           ...
           write(stdout(),9502) j, numflt(j), yt0(1,j)
       else
           numflt(j) = 0
       endif
    enddo
  ...
  do n = 1, num_prog_tracers
     ...
     if (n == itemp) then
         ...
         id_filter(n) = register_diag_field ('ocean_model', 'filter_'//trim(T_prog(n)%name), &
                    Grid%tracer_axes(1:3), Time%model_time, 'dht * polar filter tendency', &
...
!
    ...
    if(.not. polar_filter_on) return 

    if (.not. module_is_initialized ) then 
      call mpp_error(FATAL, '==>Error from ocean_polar_filter_mod (polar_filter_tracers): module must be initialized')
    endif 
        ...
        do j=jsc,jec
           if (numflt(j+Dom%joff) .ne. 0) then
              do n = 1, num_prog_tracers
                  ...
                  if (n == itemp) field_ik(:,:) = (field_ik(:,:) + tfreeze) * Grd%tmask(:,j,:)
                  call fast_fir (field_ik, Grd%tmask(:,j,:), numflt(j+Dom%joff))
                  if (n == itemp) field_ik(:,:) = (field_ik(:,:) - tfreeze) * Grd%tmask(:,j,:)
           ...
           do k=1,nk
              ...
              enddo
               T_prog(n)%wrk1(isc:iec,jsc:jec,k) = T_prog(n)%wrk1(isc:iec,jsc:jec,k)*&
                   T_prog(n)%conversion*Thickness%dht(isc:iec,jsc:jec,k,tau)
           enddo
...
!
  subroutine fast_fir (f, rmask, num)
    ...
    if (.not. module_is_initialized ) then 
      call mpp_error(FATAL, '==>Error from ocean_polar_filter_mod (fast_fir): module must be initialized')
    endif 
...
!
    ...
    if (.not. module_is_initialized ) then 
      call mpp_error(FATAL, '==>Error from ocean_polar_filter_mod (set_polar_filtering_indices): module must be initialized')
    endif 
