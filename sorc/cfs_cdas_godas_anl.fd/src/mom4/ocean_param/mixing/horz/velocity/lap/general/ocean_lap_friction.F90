...
logical :: ncar_only_pacific    = .false.  ! for use of the ncar scheme only in the tropical Pacific
real :: eq_pac_slon = 0.0                  ! start longitude for tropical Pacific
real :: eq_pac_elon = 0.0                  ! end longitude for tropical Pacific
...
character(len=256) :: version=&
     '=>Using: /general/ocean_lap_friction.f90 ($Id: ocean_lap_friction.F90,v 1.6 2005/05/12 21:41:05 gtn Exp $)'
...
namelist /ocean_lap_friction_general_nml/ lap_friction_on, bottom_5point, k_smag_iso, k_smag_aniso, &
                         ...
                         viscosity_ncar, ncar_isotropic_off_equator, ncar_only_equatorial, ncar_only_pacific, &
                         eq_pac_slon, eq_pac_elon, &
...
subroutine ocean_lap_friction_init(Grid, Domain, Time, d_time)
  ...
  if (ncar_only_pacific) ncar_only_equatorial = .TRUE.
  write (stdout(),'(/)')
  ...
  if(viscosity_ncar)  then 
    ...
    if(ncar_only_equatorial) then 
      ...
      if (ncar_only_pacific) then
        write( stdout(),'(1x,a,2f10.4)')'==> ncar_only_pacific=.true.  => long limits:',eq_pac_slon,eq_pac_elon
      endif
    endif 
  ...
  if(viscosity_ncar)  then
     ...
     if(ncar_only_pacific) then
        do j=jsd,jed
           do i=isd,ied
              if (Grd%xu(i,j) < eq_pac_slon .or. Grd%xu(i,j) > eq_pac_elon) then
                 aiso_back(i,j,:)   = vel_micom_iso  *(2.0*Grd%dxu(i,j)*Grd%dyu(i,j))/(epsln + Grd%dxu(i,j) + Grd%dyu(i,j))
                 aaniso_back(i,j,:) = vel_micom_aniso*(2.0*Grd%dxu(i,j)*Grd%dyu(i,j))/(epsln + Grd%dxu(i,j) + Grd%dyu(i,j))
              endif
           enddo
...
! bug        umagr = 1.0/(epsln + sqrt(usqrd + vsqrd))
