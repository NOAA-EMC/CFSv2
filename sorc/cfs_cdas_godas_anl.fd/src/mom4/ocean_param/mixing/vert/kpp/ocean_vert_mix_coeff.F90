...
character(len=256) :: version=&
     '$Id: ocean_vert_mix_coeff.F90,v 1.7 2005/05/12 21:41:05 gtn Exp $'
...
!                = (stf(i,j,temp) - pme*(t(i,j,1,temp,tau)-tpme(temp)) - river*(t(i,j,1,temp)-triver(temp))   <- bug fixed
!     wsfc(salt) = kinematic salinity       ..                     (PSU*m/s)
!                = stf(i,j,salt) - pme*(t(i,j,1,salt,tau)-tpme(salt)) - river*(t(i,j,1,salt)-triver(salt))    <- bug fixed
