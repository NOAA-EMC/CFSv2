module idealized_bc_mod 
  ...
  !--- version information 
  character(len=128) :: version = '$Id: idealized_bc.f90,v 1.3 2005/05/12 21:41:05 gtn Exp $'
...
10     call mpp_close (unit)
  ...
  subroutine get_grid
    ...
    if(file_exist(trim(grid_file))) then
       ...
       call read_data(trim(grid_file), 'y_C', xu)
  ...
  subroutine write_flux_data(file, flux, fld_name, fld_unit, fld_longname)
    ...
    call mpp_write(unit, fld_flux, flux)
    call mpp_close(unit)
  ...
  subroutine write_wind_data( )
    ...
    call mpp_write(unit, fld_taux,  global_taux)
    call mpp_write(unit, fld_tauy,  global_tauy)
    call mpp_close(unit)
