...
main (int argc, char *argv[])
{
  ...
  { char cmd[BUFSIZ];
    ...
    if (ofile) {
      if(iAXO >0) NC_CALL(nc_def_dim(ncid, "i_atmXocn", iAXO, &dim_AO))
    ...
    if(iAXL >0) NC_CALL(nc_def_dim(ncid, "i_atmXlnd", iAXL, &dim_AL))
    ...
    if(iLXO >0) NC_CALL(nc_def_dim(ncid, "i_lndXocn", iLXO, &dim_LO))
    ...
    NC_CALL(nc_def_var(ncid, "AREA_LND_CELL", NC_DOUBLE, 2, l_dims, 
	                                                  &lnd_cell_area_id))
    if (ofile) {
