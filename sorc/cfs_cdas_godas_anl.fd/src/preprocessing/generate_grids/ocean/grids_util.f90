module grids_util_mod
  ...
  real, parameter :: max_file_size = 2147483647.  !The maximum size in bytes for any one file. 
...
subroutine make_axis(cart, maxlen, num_regions,bounds,delta,corners,centers, &
                     num, cyclic_grid, tripolar_grid, square_grid, extend_square_grid )
    ...
    logical,                intent(in) :: cyclic_grid,  tripolar_grid
    logical,                intent(in) :: square_grid, extend_square_grid

  integer:: i, n, m, ii
  real   :: lon_start, lon_end, var, period
  ...
  case ('X')
     if( tripolar_grid ) then
        write(stdout(),*)"Modifying longitudinal grid for tripolar if needed"
        lon_start = bounds(1)
        lon_end   = bounds(num_regions)
        num_regions = 2
        ! when tripolar grid, lon_end - lon_start should equal to 360
        if(abs(lon_end-lon_start-360.) > tolr ) call mpp_error(FATAL, &
             'grids_util_mod: The longitude range should be 360 degree, modify hgrid_nml "x_lon" ')
        if(lon_start .ne. -280 .and. lon_end .ne. 80 ) then
           write(stdout(),*) ' '
           write(stdout(),*)' WARNING from grids_util_mod: the grid is tripolar grid, but ', &
                            ...
                            'the two poles: ', lon_start, ', ', lon_end
           write(stdout(),*) ' '
        endif
        bounds(1) = lon_start
        bounds(2) = lon_end
        delta(2) =  delta(1)
        if (mod(360.,delta(1)) .ne. 0) call mpp_error(FATAL, &
             'grid_util_mod: non-integral number of longitude cells. modify hgrid_nml "dx_lon"')
     endif

     if( cyclic_grid ) then
        period = bounds(num_regions) - bounds(1)
        write(stdout(),*) "NOTE from grid_util: the x-direction period is ",period
        if ( delta(num_regions) .ne. delta(1) ) call mpp_error(FATAL, &
             ' grids_util_mod: dx_lon(1) must equal dx_lon(last), modify hgrid_nml "dx_lon" ' )
     endif
  case ('Y')
     if(square_grid) then
     ...
     if (tripolar_grid) then
        if ( bounds(num_regions) < 90.) call mpp_error(FATAL, &
             "grids_util_mod: the last bound is less than 90 degree and is tripolar grid")
     endif
     if ( bounds(num_regions) - bounds(1) > 180.0) write(stdout(),*)   &
          "=>Warning: Latitudinal domain width exceeds 180 deg.", &
          "Restricting last bounds to ",delta(num_regions) +180.0
   end select 
