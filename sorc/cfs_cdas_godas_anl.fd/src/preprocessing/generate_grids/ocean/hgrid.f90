module hgrid_mod
  ...
  !</DATA>
  !<DATA NAME="channel_grid" TYPE="logical">
  ! When true, it will generate channel grid. Default value is false. When channel_grid is true,
  ! tripolar_grid should be false.
  !</DATA>
  ...
  namelist /hgrid_nml/ nxlons, nylats, x_lon, dx_lon, y_lat, dy_lat, square_grid, &
       extend_square_grid, cyclic, tripolar_grid, lat_join, read_my_grid,         &
       my_grid_file, debug, channel_grid
  ...
  real                            :: period = 0.0               ! period of longitude.
  ...
  !---------version information-------------------------------------------
  character(len=128) :: version = '$Id: hgrid.f90,v 1.6 2005/05/12 21:41:05 gtn Exp $'
...
contains
  ...
  subroutine hgrid_init 
    ...
    !--- local variables -------------------------------------------------
    integer                     :: i, j, k, n, num, pe, unit, ierr, io, npes
...
10     call close_file (unit)
    ...
    if(.not. read_my_grid) then
       if(channel_grid .and. tripolar_grid) call mpp_error(FATAL, &
            'hgrid_mod: nml "channel_grid"= true contradicts with nml "tripolar_grid"=true')
    ...
    if(cyclic) then
       period = 360.0
       if(channel_grid) period = x_lon(nxlons) - x_lon(1)
    endif
    ...
    if (read_my_grid) then
       ...
       if(my_grid_file(len-2:len) == '.nc') then
          ...
          if(.not. found_x_vert_t) call mpp_error(FATAL,'field x_vert_T does not exist in file '//trim(my_grid_file) )
          if(.not. found_y_vert_t) call mpp_error(FATAL,'field y_vert_T does not exist in file '//trim(my_grid_file) )
       ...
       if (cyclic) then  ! in this case, we alway suppose the period is 360.
          xt0(0) = xt0(ni)-360.0
          xt0(ni+1) = xt0(1)+360.0
          xu0(ni+1) = xu0(0)+360.0
       else
       ...
       write (stdout(),'(/a)')'Generating longitudinal resolution for the computational domain:'
        call make_axis('X',maxlen, nxlons,x_lon,dx_lon,xu0,xt0,ni, &
                      cyclic, tripolar_grid, square_grid, extend_square_grid)
       write (stdout(),'(/a)')'Generating latitudinal resolution for the computational domain:'
       call make_axis('Y',maxlen, nylats,y_lat,dy_lat,yu0,yt0,nj, &
                      cyclic, tripolar_grid, square_grid, extend_square_grid )
    endif
...
9108 format (/,' Longitude of C points (deg): xu(i) i=1,',i4)
  ...
  subroutine spherical_grid_init(Hgrid)
    ...
    call cell_length_area(Hgrid%T, Hgrid%T%x(isc:iec,jsc:jec), Hgrid%T%y(isc:iec,jsc:jec),             &
    ...
    call cell_length_area(Hgrid%E, Hgrid%E%x(isc:iec,jsc:jec), Hgrid%E%y(isc:iec,jsc:jec),             &
    ...
    call cell_length_area(Hgrid%N, Hgrid%N%x(isc:iec,jsc:jec), Hgrid%N%y(isc:iec,jsc:jec),             &
    ...
    call cell_length_area(Hgrid%C, Hgrid%C%x(isc:iec,jsc:jec), Hgrid%C%y(isc:iec,jsc:jec),             &
  ...
  subroutine write_cell_data(file, cell, cell_id)
    ...
    call write_field_data(file, 'x_'//cell_id, cell%x(1:ni,1:nj))
    call write_field_data(file, 'y_'//cell_id, cell%y(1:ni,1:nj))  
  ...
  function box_area(x1,y1,x2,y2,x3,y3,x4,y4)
    ...
    do i = 1, n
       ...
       if (lat1 == lat2) then ! cheap area calculation along latitude 
          box_area = box_area - dx*sin(lat1)
       else
          box_area = box_area - dx*(sin(lat1)+sin(lat2))/2   !  TRAPEZOID_RULE
       endif
    enddo

    box_area = box_area * radius * radius
  ...
  !#######################################################################
  !calculate face length and half length of each cell.
  subroutine cell_length_area(cell, x, y, x_e, y_e, x_n, y_n, x_c, y_c)
    type(cell_type),                intent(inout) :: cell
    ...
    do j=jsc,jec
       do i=isc,iec
          area1 = box_area(x_c(i-1,j-1), y_c(i-1,j-1), x_n(i,j-1), y_n(i,j-1), &
               x(i,j), y(i,j), x_e(i-1,j), y_e(i-1,j)  )
          area2 = box_area(x_n(i,j-1), y_n(i,j-1), x_c(i,j-1), y_c(i,j-1), &
               x_e(i,j), y_e(i,j), x(i,j), y(i,j)  )
          area3 = box_area(x(i,j), y(i,j), x_e(i,j), y_e(i,j), &
               x_c(i,j), y_c(i,j), x_n(i,j), y_n(i,j)  )
          area4 = box_area(x_e(i-1,j), y_e(i-1,j), x(i,j), y(i,j), &
  ...
  end subroutine cell_length_area
