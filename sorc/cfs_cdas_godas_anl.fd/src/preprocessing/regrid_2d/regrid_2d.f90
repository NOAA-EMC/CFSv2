program regrid_2d
  ...
  ! <REVIEWER EMAIL="Bonnie.Samuels@noaa.gov">Bonnie Samuels</REVIEWER>
  !
  !<DESCRIPTION>
  ...
  character(len=128) :: version='CVS $Id: regrid_2d.f90,v 1.5 2005/05/12 21:41:05 gtn Exp $'
  character(len=128) :: tagname='Tag $Name: mom4p0d $'
...
contains
  ...
  subroutine read_dst_grid
    ...
    do i=1,natt
       if (global_atts(i)%name == 'y_boundary_type') y_boundary_type = global_atts(i)%catt(1:global_atts(i)%len)
       if (global_atts(i)%name == 'filename') title = global_atts(i)%catt(1:global_atts(i)%len)
    enddo
