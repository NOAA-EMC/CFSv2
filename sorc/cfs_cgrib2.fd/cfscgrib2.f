...
C  
C MAIN PROGRAM:  cfscgrib2
C   PRGMMR: Vuong          ORG: SIB         DATE: 2010-12-06
C
...
C 2010-12-06  Vuong    Modified original program cfscgrib2 for CFS and
C                      CFS RR products
...
C     UNIQUE:    - cnvcfs12, cnvcfs21, cnv22, usage
C     LIBRARY:
C       W3LIB    - errexit
...
C  GET ARGUMENTS
      ...
      IF(NARG.lt.3) THEN       ! may be a problem with args
         IF(NARG.eq.0) THEN
            !CALL ERRMSG('cfscgrib2:  Incorrect usage')
            call usage(0)
            CALL ERREXIT(2)
     ...
     &      table_ver .gt. mastertab ) then
            call usage(0)
            call errmsg ('  ')
            call errmsg('cfscgrib2: cannot change to master table '//
     ...
     &                  curmastertab_ver)
         ...
         if ( (ipack.ne.-1).and.(outver.eq.1) ) then
            CALL ERRMSG('cfscgrib2: -pxx option ignored when '//
     &                  'using -g21')
         endif
         !
         !   Must have -g option
         ...
         if ( (inver.eq.0).or.(outver.eq.0) ) then
            CALL ERRMSG('cfscgrib2: must use one -gxx option')
            call usage(0)
            CALL ERREXIT(2)
         endif
         !
     ...
     &                       ipack.ne.32) ) then
            CALL ERRMSG('cfscgrib2: -m or -m0 option ignored when '//
     &                  'not using -p2, -p31 or -p32.')
            usemiss=.false.
         endif
      ENDIF
!
...
!
      ...
      if (IOS.NE.0) then
         call errmsg('cfscgrib2: cannot open input GRIB file '//
     &               gfilein(1:NCGB))
         call errexit(3)
      endif
      ...
      if (IOS.NE.0) then
         call errmsg('cfscgrib2: cannot open output GRIB file '//
     &               gfileout(1:NCGB))
         call errexit(4)
      endif
!
...
!
      if ((inver.eq.1).AND.(outver.eq.2)) then
         call cnvcfs12(ifl1,ifl2,ipack,usemiss,imiss,uvvect, table_ver)
      elseif ((inver.eq.2).AND.(outver.eq.1)) then
         call cnvcfs21(ifl1,ifl2)
...
C PROGRAM HISTORY LOG:
C 2003-06-06  Gilbert
C 2010-12-02  Vuong   -  Changed the cfscgrib2_ver
C
...
C
C$$$
         character(len=16) :: cfscgrib2_ver="cfscgrib2-1.2.2"
         integer,intent(in) :: iopt 

         if ( iopt.eq.0 ) then
         call errmsg ('  ')
         call errmsg('Usage: cfscgrib2 [-h] {-g12|-g21|-g22} [-m|-m0]'//
     &        ' [-nv] [-mastertable_ver_x]')
         call errmsg('               [{-p0|-p2|-p31|-p32|-p40'//
     &               '|-p41}]  ingribfile   outgribfile')
         call errmsg ('  ')
         call errmsg('Usage: cfscgrib2  -h  For helps and shows all'//
     &                                  ' options')
         call errmsg ('  ')
         endif
         ...
         if ( iopt.eq.1 ) then
            call errmsg ('  ')
            call errmsg('cfscgrib2:  version '//cfscgrib2_ver)
            call errmsg ('  ')
     ...
     &               ' where x is number from 2 to 10')
         call errmsg ('  ')
         endif
      return
      end
