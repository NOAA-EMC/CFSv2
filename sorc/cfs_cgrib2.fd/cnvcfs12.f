      subroutine cnvcfs12(ifl1,ifl2,ipack,usemiss,imiss,uvvect,
     &                    table_ver)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:    cnvcfs12 
...
C 2010-12-02  Vuong    - Add option -mastertable_ver_x where x is mater table
C                        version 2 to 10
C
C USAGE:    CALL cnvcfs12(ifl1,ifl2,ipack)
...
!      
      ...
      do 
        ...
        if (lgrib.eq.lengrib) then
           ...
           numpts=KPTR(10)
           if (iret.ne.0) then
              print *,' cfscgrib2: Error unpacking GRIB field.',iret
              iseek=lskip+lgrib
              cycle
           endif
        else
           print *,' cfscgrib2: IO Error on input GRIB file.'
           cycle
        endif
        iseek=lskip+lgrib
...
! set PDS Template
     ...
     &                     ipdsnum,ipdstmpl,numcoord,coordlist,ierr)
          ...
          call pds2pdtcfs(kpds,ipdsnum,ipdstmpl,numcoord,coordlist,ierr)
        endif
        if (ierr.ne.0) then
          cycle
        endif
