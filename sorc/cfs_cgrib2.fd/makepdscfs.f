        subroutine makepdscfs(idisc,idsect,ipdsnum,ipdstmpl,ibmap,
     &                     idrsnum,idrstmpl,kpds,iret)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:    makepdscfs
...
C
C USAGE:    CALL makepdscfs(idisc,idsect,ipdsnum,ipdstmpl,ibmap,
...
C$$$
        ...
        if ( (ipdsnum.lt.0).OR.(ipdsnum.gt.14) ) then
           print *,'makepdscfs: Don:t know GRIB2 PDT 4.',ipdsnum
           iret=2
           return
        endif
        ...
        if ( ibmap.ge.1.AND.ibmap.le.253 ) then
           print *,'makepdscfs: Don:t know about predefined bit-map ',ibmap
           iret=1
           return
        endif
     ...
     &            ipdstmpl(5).eq.121) kpds(7)=1
           ...
           selectcase (ipdstmpl(ipos))
            ...
            case (195)
              kpds(16)=128
            case (196)
              kpds(16)=129                           ! Time Range Indicator
            case (197)
              kpds(16)=130
            case (198)
              kpds(16)=131
            ...
            if ((ipdstmpl(5).eq.82 .OR. ipdstmpl(5).eq.98)
     &         .AND. (ipdstmpl(ipos).eq.0 .OR.
     &         ipdstmpl(ipos).eq.255 .OR. ipdstmpl(ipos).eq.1)) then
                 if (ipdstmpl(ipos).eq.0) kpds(16)=3        ! Time Range Indicator
                 if (ipdstmpl(ipos).eq.255) kpds(16)=2      ! Time Range Indicator
                 if (ipdstmpl(ipos).eq.1) kpds(16)=4        ! Time Range Indicator
            end if
            if (ipdstmpl(27).eq.0) then
                kpds(17)=0                         
            else
                if ((ipdstmpl(5).eq.82.OR.ipdstmpl(5).eq.199)
     &             .AND.ipdstmpl(ipos).eq.0) then
                   kpds(16)=10
                end if
                kpds(17)=ipdstmpl(27)                  ! Number of Average
            endif
            kpds(15)=ipdstmpl(ipos+5)
            if (ipdstmpl(5).eq. 197) then
               kpds(15)=ipdstmpl(ipos+3)+kpds(14)
               kpds(17)=0
            end if
...
C$$$
        ...
        if ( ltype1.eq.1.AND.ltype2.eq.255 ) then
           ltype=1
