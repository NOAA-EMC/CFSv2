     ...
     x    (direction,
     x     trie_q,trio_q,trie_te,trio_te,trie_rq,trio_rq,
...
!     integer, parameter  ::   lotx = levs+levh+1
      ...
      real(kind=kind_evod) for_gr_r_2(lonrx*(levs+levh+1),lats_dim_r)
!
cc
...
cc
      integer              i,j,k,kap,kat,kar,kk,nn,nnl
...
cc ------------------------ transform back to coefficient ----
     ...
     &                  lon_dim,lons_lat,lonrx,levs+1)
 
      enddo
!
      dimg=0
      call four2flnio(lslag,lats_dim_r,lotx,levs+1,for_gr_r_1,
     x              ls_nodes,max_ls_nodes,
