...
!    &                     rbint,   rdt,    rdz,    rdzt1,
     ...
     &,                    xkzm_loc(im)
cc
!     real (kind=kind_phys), parameter :: xkzm_min=3.0
!     real (kind=kind_phys), parameter :: xkzm_min=2.0
      real (kind=kind_phys), parameter :: xkzm_min=1.0
!     real (kind=kind_phys), parameter :: xkzm_min=0.5
!     real (kind=kind_phys), parameter :: xkzm_min=0.75
!     real (kind=kind_phys), parameter :: xkzm_min=0.25
!     real (kind=kind_phys), parameter :: xkzm_min=0.10
...
!     PARAMETER(QMIN=1.E-8,XKZM=1.5,ZFMIN=1.E-8,APHI5=5.,APHI16=16.)
...
!       xkzm_loc(i) = max(xkzm_min, min(xkzm, oro(i)*0.001))
        xkzm_loc(i) = min(xkzm, xkzm_min + oro(i)*0.002)
      ENDDO
      DO K = 1,KM-1
        DO I=1,IM
...
!         if (k .le. kinver(i)) then
            tem1      = 1.0 - prsi(i,k+1) / prsi(i,1)
!Nogod      tem1      = max(0.0, 1.0 - prsi(i,k+1) * 0.01)
!           tem1      = tem1 * tem1 * 5.0
            tem1      = tem1 * tem1 * 10.0
!           tem1      = tem1 * tem1 * 20.0
!           xkzo(i,k) = xkzm * min(1.0, exp(-tem1))
