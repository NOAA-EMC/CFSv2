...
!!
      ...
      integer nosig,nosfc,noflx,nfill
      character*16 cosfc
!     data timesum/0./
!!
...
!
      ...
      if( thermodyn_id.eq.3 .and. sfcpress_id.eq.2
...
!       call mpi_gather4(trieo_ls_node , lenrec, MPI_R_MPI,
...
!*    PARAMETER(NFLD=18)
!*    PARAMETER(NFLD=25)
      PARAMETER(NFLD=29)       ! yth 3/10 add 4 sw flux components
...
!jmeng add snowfall rate
!     integer, parameter :: ISRWEQ=64
...
!  ---  sw downward surface clux components  - yth 03/10
...
!         if (ierr/=0 .and. l==34) print*,'wrtsfc gribit ierr=',ierr,   &
!    &   '  105)Downward sw uv+vis beam radiation flux (W/m**2) SFC '
!         if (ierr/=0 .and. l==35) print*,'wrtsfc gribit ierr=',ierr,   &
!    &   '  106)Downward sw uv+vis diffuse radiation flux (W/m**2) SFC'
!         if (ierr/=0 .and. l==36) print*,'wrtsfc gribit ierr=',ierr,   &
!    &   '  107)Downward sw nir beam radiation flux (W/m**2) SFC '
!         if (ierr/=0 .and. l==37) print*,'wrtsfc gribit ierr=',ierr,   &
!    &   '  108)Downward sw nir diffuse radiation flux (W/m**2) SFC '
      enddo

!jmeng add snowfall rate

      glolal = flx_fld%GESHEM*1.E3*RTIME*sfc_fld%srflag

      call uninterprez(2,kmsk0,buffo,glolal,global_lats_r,lonsperlar)

!     if (ierr /= 0) print*,'wrtsfc gribit ierr=',ierr,'  ',            &
!    &  '109)Snow precipitation rate (kg/m**2/s) land sea surface '
...
!!
      integer   IPRS,ITEMP,IZNLW,IMERW,ISPHUM,IPWAT,
     $          IPCPR,ISNOWD,ICLDF,ICCLDF,
     $          ISLMSK,IZORL,IALBDO,ISOILM,ISNOHF,ICEMSK,
     $          ILHFLX,ISHFLX,IZWS,IMWS,IGHFLX,
     $          IUSWFC,IDSWFC,IULWFC,IDLWFC,
     $          INSWFC,INLWFC,
     $          IDSWVB,IDSWVD,IDSWNB,IDSWND,
...
!Clu [+1L]: define parameter index, using Table 130
      PARAMETER(ISLC=160,ISNOD=66)
!Cwei
      PARAMETER(ISLO=222,ISBS=198,ISNC=238,ICMM=179,ISNOHF=229)
!Clu [+1L]: define parameter index, using Table 2
      PARAMETER(ICNP=223)
!Cwei
...
!    +                ICSULW/
     &                ICSULW,IDSWVB, IDSWVD, IDSWNB, IDSWND /        ! yth 3/10
...
!    +               ISFC /
     &               ISFC, ISFC, ISFC, ISFC, ISFC /                  ! yth 3/10
!    $               ILCLYR, ILCTL , ILCBL , ILCTL /
      INTEGER     IDATE(4), IDS(255), ids_iq
!     INTEGER     IDATE(4), IDS(255),IENS(5)
      real (kind=kind_io8) SI(LEVP1)
C
csela..................................................................
...
!     ids(IQMX)   = 5
!     ids(IQMN)   = 5
      ids_iq      = 5                         ! Moorthi on 02/01/2013
! UV-B scaling factor, if set up already, comment the next 2 lines out
      ids(IUVBF)  = 2
      ids(IUVBFC) = 2
! Ice conentration and thickness scaling factor
...
!wei added 10/24/2006
      ...
      ids(ISNOD)  = 6
!     ids(ISNOWD) = 5
      ids(ISNOWD) = 3                      ! Moorthi 02/01/2013
      ids(ICNP)   = 5
      ids(IPCPR)  = 6
      ids(ICPCPR) = 6
      ids(IRNOF)  = 5
...
!     glolal=SNWDPH / 1.E3       !! convert from mm to m
      ...
      if(me.eq.ioproc) then
      LBM=slmskful.EQ.1._kind_io8
!     LBM=slmskful.EQ.1._kind_io8 .or. slmskful.EQ.2._kind_io8 !As in GFS
      call gribit(wrkga,LBM,4,lonr,latr,16,CL1,ILPDS,2,ICEN,IGEN,
...
!       enddo
     ...
     &                0.,0.,0.,0.,0.,0.,G,LG,IERR)
          ...
          if (ierr == 0 ) then
            ...
            if (l == 34) print*,'wrtsfc gribit ierr=',ierr,             &
     &   '  105)Downward sw uv+vis beam radiation flux (W/m**2) SFC '
            if (l == 35) print*,'wrtsfc gribit ierr=',ierr,             &
     &   '  106)Downward sw uv+vis diffuse radiation flux (W/m**2) SFC'
            if (l == 36) print*,'wrtsfc gribit ierr=',ierr,             &
     &   '  107)Downward sw nir beam radiation flux (W/m**2) SFC '
            if (l == 37) print*,'wrtsfc gribit ierr=',ierr,             &
     &   '  108)Downward sw nir diffuse radiation flux (W/m**2) SFC '
          endif
        endif
      enddo
...
!     glolal=flx_fld%GESHEM*1.E3*RTIME*sfc_fld%srflag
      ...
      if (me == ioproc) then
        call gribit(wrkga,LBM,4,lonr,latr,16,CL1,ILPDS,130,ICEN,IGEN,   &
     &              0,ISRWEQ,ISFC,0,0,IYR,IMO,IDA,IHR,                  &
     &              IFHOUR,IFHR,ITHR,IAVG,0,0,ICEN2,IDS(ISRWEQ),IENS,   &
     &              0.,0.,0.,0.,0.,0.,G,LG,IERR)
        if (ierr == 0) then
          CALL WRYTE(noflx,LG,G)
        else
          print*,'wrtsfc gribit ierr=',ierr,'  ',                       &
     x '109)Snow precipitation rate (kg/m**2/s) land sea surface '
        endif
      endif
