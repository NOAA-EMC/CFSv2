...
!JFE &       SYN_GR_A_2,DYN_GR_A_2,ANL_GR_A_2)
        ELSE
          global_times_a=0.
!     write(0,*)' in do_tstep calling gloopa kdt=',kdt
          CALL GLOOPA
...
!      write(0,*)'COMPLETED ATM_GETSSTICE IN do_tstep kdt=',kdt
...
!     print *,' after gloopr sdec=',sdec,' cdec=',cdec
       ENDIF !not.adiab
      ENDIF
!      write(0,*)'COMPLETED GLOOPR IN do_tstep kdt=',kdt
!!
...
!      write(0,*)'before diffusion IN do_tstep kdt=',kdt
!
!----------------------------------------------------------
...
!      write(0,*)'before gloopb IN do_tstep kdt=',kdt
      if(.not.adiab) then
     ...
     X     SNNP1EV,SNNP1OD,NDEXEV,NDEXOD,
     X     PLNEV_R,PLNOD_R,PDDEV_R,PDDOD_R,PLNEW_R,PLNOW_R,
     &     deltim,PHOUR, sfc_fld, flx_fld, SFALB,
     &     XLON  ,
...
!--------------------------------------------
!--------------------------------------------
!      write(0,*)'before wrtout IN do_tstep kdt=',kdt
      IF (lsout) THEN
!!
...
!     endif
!
!     write(0,*)' after returning from wrtout at fhour=',fhour
      CALL f_hpmstop(32)
!!
...
!
        tag = kdt
!       write(0,*)' me, nodes,kdt = ',me,nodes,kdt
        if (me .eq. 0) then
...
!
!     write(0,*)' before flx_init ',fhour,' kdt=',kdt,' nszer=',nszer
...
!     write(0,*)' before ATM_SENDFLUXES ',fhour
! Coupling insertion->
      CALL ATM_SENDFLUXES(sfc_fld%SLMSK)
!<- Coupling insertion

!     write(0,*)' after ATM_SENDFLUXES ',fhour
      RETURN
      END
