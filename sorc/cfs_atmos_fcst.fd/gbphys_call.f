...
!  ---  inputs:
     ...
     &      hprime_v, xlon(istrt,lan),xlat(istrt,lan),                  &
     &      sfc_fld%slope (istrt,lan),    sfc_fld%shdmin(istrt,lan),    &
     &      sfc_fld%shdmax(istrt,lan),    sfc_fld%snoalb(istrt,lan),    &
     &      sfc_fld%tg3   (istrt,lan),    sfc_fld%slmsk (istrt,lan),    &
     &      sfc_fld%vfrac (istrt,lan),    sfc_fld%vtype (istrt,lan),    &
     &      sfc_fld%stype (istrt,lan),    sfc_fld%uustar(istrt,lan),    &
     &      sfc_fld%oro   (istrt,lan),                                  &
     &      flx_fld%coszen(istrt,lan),    flx_fld%sfcdsw(istrt,lan),    &
     &      flx_fld%sfcnsw(istrt,lan),    flx_fld%sfcdlw(istrt,lan),    &
     &      flx_fld%tsflw (istrt,lan),    flx_fld%sfcemis(istrt,lan),   &
     &      sfalb         (istrt,lan),                                  &
     &      swh(1,1,iblk,lan), hlw(1,1,iblk,lan), ras, pre_rad,         &
     &      ldiag3d, trans_trac, lssav, lssav_cc, bkgd_vdif, flipv,     &
     &      old_monin, cnvgwd, sashal, newsas, mom4ice, mstrat,         &
!  ---  input/outputs:
     &      sfc_fld%hice  (istrt,lan),    sfc_fld%fice  (istrt,lan),    &
     &      sfc_fld%tisfc (istrt,lan),    sfc_fld%tsea  (istrt,lan),    &
     &      sfc_fld%tprcp (istrt,lan),    sfc_fld%cv    (istrt,lan),    &
     &      sfc_fld%cvb   (istrt,lan),    sfc_fld%cvt   (istrt,lan),    &
     &      acv           (istrt,lan),    acvb          (istrt,lan),    &
     &      acvt          (istrt,lan),    flx_fld%cldwrk(istrt,lan),    &
     &      phy_f3dv, phy_f2dv,           flx_fld%ep    (istrt,lan),    &
     &      slc_v, smc_v, stc_v,          upd_mf     (1,1,iblk,lan),    &
     &      dwn_mf     (1,1,iblk,lan),    det_mf     (1,1,iblk,lan),    &
     &      sfc_fld%srflag(istrt,lan),    sfc_fld%snwdph(istrt,lan),    &
     &      sfc_fld%sheleg(istrt,lan),    sfc_fld%sncovr(istrt,lan),    &
     &      sfc_fld%zorl  (istrt,lan),    sfc_fld%canopy(istrt,lan),    &
     &      sfc_fld%ffmm  (istrt,lan),    sfc_fld%ffhh  (istrt,lan),    &
     &      sfc_fld%f10m  (istrt,lan),    flx_fld%srunoff(istrt,lan),   &
     &      flx_fld%evbsa (istrt,lan),    flx_fld%evcwa (istrt,lan),    &
     &      flx_fld%snohfa(istrt,lan),    flx_fld%transa(istrt,lan),    &
     &      flx_fld%sbsnoa(istrt,lan),    flx_fld%snowca(istrt,lan),    &
     &      flx_fld%soilm (istrt,lan),    flx_fld%tmpmin(istrt,lan),    &
     &      flx_fld%tmpmax(istrt,lan),    flx_fld%dugwd (istrt,lan),    &
     &      flx_fld%dvgwd (istrt,lan),    flx_fld%psmean(istrt,lan),    &
     &      flx_fld%bengsh(istrt,lan),    flx_fld%spfhmin(istrt,lan),   &
     &      flx_fld%spfhmax(istrt,lan),   flx_fld%dusfc (istrt,lan),    &
     &      flx_fld%dvsfc (istrt,lan),    flx_fld%dtsfc (istrt,lan),    &
     &      flx_fld%dqsfc (istrt,lan),    flx_fld%geshem(istrt,lan),    &
     ...
     &      flx_fld%dlwsfc(istrt,lan),    flx_fld%ulwsfc(istrt,lan),    &
     &      dlwsfc_cc     (istrt,lan),    ulwsfc_cc     (istrt,lan),    &
     &      swsfc_cc      (istrt,lan),    dusfc_cc      (istrt,lan),    &
     &      dvsfc_cc      (istrt,lan),    dtsfc_cc      (istrt,lan),    &
     &      dqsfc_cc      (istrt,lan),    precr_cc      (istrt,lan),    &
!  ---  outputs:
     &      adt, adr, adu, adv,                                         &
     &      sfc_fld%t2m   (istrt,lan),    sfc_fld%q2m   (istrt,lan),    &
     &      flx_fld%u10m  (istrt,lan),    flx_fld%v10m  (istrt,lan),    &
     &      flx_fld%zlvl  (istrt,lan),    flx_fld%psurf (istrt,lan),    &
     ...
     &      flx_fld%dlwsfci(istrt,lan),   flx_fld%ulwsfci(istrt,lan),   &
     &      flx_fld%dswsfci(istrt,lan),   flx_fld%uswsfci(istrt,lan),   &
     &      flx_fld%dtsfci(istrt,lan),    flx_fld%dqsfci(istrt,lan),    &
     &      flx_fld%gfluxi(istrt,lan),    flx_fld%epi   (istrt,lan),    &
     &      xmu_cc        (istrt,lan),    dlw_cc        (istrt,lan),    &
     &      dsw_cc        (istrt,lan),    snw_cc        (istrt,lan),    &
     &      lprec_cc      (istrt,lan)                                   &
     &    )
