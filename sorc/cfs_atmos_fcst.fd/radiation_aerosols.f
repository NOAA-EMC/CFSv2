...
!      sigref(NDM,NAE)  - ref sigma lev (sfc to toa)
      ...
      real (kind=kind_phys), allocatable, save, dimension(:,:) :: sigref
...
!  --- ...  convert pressure reference level to sigma reference level
...
!test real (kind=kind_phys), dimension(IMAX,NLAY) :: aersav
      ...
      real (kind=kind_phys) :: tmp1, tmp2, rps
...
!       convert pressure level to sigma level to follow the terrain
     ...
     &              ' unrealistic surface pressure =', prsi(i,1)
          ...
          i1 = 1
          do k = 1, NLAY
            if (prsi(i,k+1)*rps < sigref(i1,kp)) then
              i1 = i1 + 1
...
!       convert pressure level to sigma level to follow the terrain
     ...
     &              ' unrealistic surface pressure =', prsi(i,NLP1)
          ...
          i1 = 1
          do k = NLAY, 1, -1
            if (prsi(i,k)*rps < sigref(i1,kp)) then
              i1 = i1 + 1
              if (i1 == 2 .and. prsref(2,kp) == prsref(3,kp)) then
                i1 = 3
              endif
