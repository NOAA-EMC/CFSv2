...
!!    type (proflw_type),    dimension(IM,LM+1) :: flwprf
     ...
     &       mbota(IM,3), mtopa(IM,3), LP1, nb, i5
...
!     if ( prnco2 .and. me==0 ) then
!       print *,' CO2 data check for month, year:', monthd, iyear0
!       i5 = min(im,5)
!       print *, (gasvmr(i,1,1),i=1,i5)
!     endif
...
!  ---  sw sfc flux components
              fluxr(i,34) = fluxr(i,34) + scmpsw(i)%visbm * tem0d  ! uv/vis beam sw dn
              fluxr(i,35) = fluxr(i,35) + scmpsw(i)%visdf * tem0d  ! uv/vis diff sw dn
              fluxr(i,36) = fluxr(i,36) + scmpsw(i)%nirbm * tem0d  ! nir beam sw dn
              fluxr(i,37) = fluxr(i,37) + scmpsw(i)%nirdf * tem0d  ! nir diff sw dn
            endif
          enddo
        endif
