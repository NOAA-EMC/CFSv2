...
C$$$
     ...
     $ 'POE QOE TOE ZOE WOE PWE PW1E PW2E PW3E PW4E NUL  NUL      '/
      ...
      NAMELIST /PREVDATA/DOFCST,DOANLS,DOVTMP,SOME_FCST,DOBERR,
     $          QTOP_REJ,SATMQC,ADPUPA_VIRT,nbax,span,fits
 ...
 7701 FORMAT(/' --> GBLEVENTS: POSTEVENTS MODE - DATE CHECK AND ',
     $ 'TRANSFORM THE ANALYSIS'/)
         ...
         IF(DOFCST .OR. SOME_FCST .OR. DOANLS) then
            ...
            DO KBAK=1,NBAK
            if(kbak==myid+1.or.nprc==1) then
               IUNITF=KBAK+19
               idhr=tspan*(kbak-(nbak+1)/2)        
               CALL ADDATE(IDATEP,idhr,IDATEC)
               if(kbak==4) idatec=idatep
               CALL GBLEVN10(IUNITF,IDATEC,IM,JM,kbak)
               CALL GBLEVN12(KBAK) ! copies one background time level to hterpt arrays
               deallocate(iar12z,iar13p,iarpsl,iarpsi,vcoord)
               deallocate(iar14t,iar15u,iar16v,iar17q)
            endif
            lug=kbak+29
            call readsf(lug,idatec,kbak) ! read surface fields
            ENDDO
