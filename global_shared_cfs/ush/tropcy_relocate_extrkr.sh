...
case ${cmodel} in 
  ...
  nam) set +x; echo " "; echo " ++ operational Early NAM chosen"; set -x;
        ...
        model=6;;
  nvgm) set +x; echo " "; echo " ++ operational NAVGEM chosen"; set -x;
		fcsthrsnvgm=' 00 12 24 36 48 60 72';
		#nvgmdir=/com/hourly/prod/hourly.${CENT}${symd};
		nvgmdir=$COMIN;
		nvgmgfile=fnoc.t${dishh}z;
        model=7;;
...
FORT31=${vdir}/vitals.${symd}${dishh}
FORT51=${vdir}/vitals.upd.${cmodel}.${symd}${dishh}
...
if [ ${model} -eq 7 ]; then
  ...
  if [ -s ${vdir}/nvgmgribfile.${symd}${dishh} ]; then
    rm ${vdir}/nvgmgribfile.${symd}${dishh}
  fi

  if [ ! -s ${nvgmdir}/${nvgmgfile} ]; then
    set +x
    echo " "
    echo " !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    echo " !!! NAVGEM File missing: ${nvgmdir}/${nvgmgfile}"
    echo " !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    echo " "
  ...
  gfile=${nvgmdir}/${nvgmgfile}
  $wgrib -s $gfile >$TMPDIR/nvgm.ix

  for fhour in ${fcsthrsnvgm}
  do
    ...
    do
      case ${parm} in
        "SurfaceU")
          grep "UGRD:19 m " $TMPDIR/nvgm.ix | grep ${vtstring} | \
            $wgrib -s $gfile -i -grib -append -o ${vdir}/nvgmgribfile.${symd}${dishh} ;;
        "SurfaceV")
          grep "VGRD:19 m " $TMPDIR/nvgm.ix | grep ${vtstring} | \
            $wgrib -s $gfile -i -grib -append -o ${vdir}/nvgmgribfile.${symd}${dishh} ;;
                 *)
          grep "${parm}" $TMPDIR/nvgm.ix | grep ${vtstring} | \
            $wgrib -s $gfile -i -grib -append -o ${vdir}/nvgmgribfile.${symd}${dishh} ;;
      esac
    done
  ...
  $gix ${vdir}/nvgmgribfile.${symd}${dishh} ${vdir}/nvgmixfile.${symd}${dishh}
  gribfile=${vdir}/nvgmgribfile.${symd}${dishh}
  ixfile=${vdir}/nvgmixfile.${symd}${dishh}
...
FORT12=${vdir}/vitals.upd.${cmodel}.${symd}${dishh}
FORT61=${vdir}/trak.${cmodel}.all.${symdh}
FORT62=${vdir}/trak.${cmodel}.atcf.${symdh}
FORT63=${vdir}/trak.${cmodel}.radii.${symdh}
FORT64=${vdir}/trak.${cmodel}.atcfunix.${symdh}
