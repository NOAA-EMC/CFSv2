...
#
# $Id: link_crtm_coeffs.sh 60129 2015-08-13 16:57:33Z Michael.Lueken@noaa.gov $

script_id()
{
  REVISION='$Revision: 60129 $'
  LAST_CHANGED_DATE='$LastChangedDate: 2015-08-13 16:57:33 +0000 (Thu, 13 Aug 2015) $'
  echo
...
for COEFF_DIR in ${ALL_COEFF_DIRS}; do
  ...
  if [[ -d ${SOURCE_DIR}/${COEFF_DIR} ]]; then
     link_coeff_files "${SOURCE_DIR}/${COEFF_DIR}/${ENDIAN_TYPE}"
  else
     link_coeff_files "${SOURCE_DIR}/${ENDIAN_TYPE}"
  fi
  if [ $? -ne 0 ]; then
