...
!
      ...
      real    es, shs, w, dtemp, tvu, tvd, tdry, psfcm
...
!     if( i == 16 .and. j  == 37) write(6,*)' hp=',hp(i,j,1),hp(i,j,kmax)
!    &,' coef3=',coef3
          DO N=2,NMAX-1
            K=(N-1)/2+1
            HP(I,J,N)=H(I,J,K)+
     &          T(I,J,K)/GAMA*(1.-(P(N)/PSIG(I,J,K))**COEF3)
!     if( i == 16 .and. j  == 37) write(6,*)' hp=',hp(i,j,n),'T=',T(i,j,n)
!    &,' p=',p(n),' psig=',psig(i,j,k),' n=',n,' k=',k
          END DO
        END DO
      END DO
...
!        write(6,*)'Press1=',N,K,P(N),HP(20,20,N),H(20,20,K)
      END DO
...
!
      ...
      real    es, shs, w, dtemp, tdry, psfcm, sh_e
...
! above top
      ...
      END
        ...
        PARAMETER (IX=41,JX=41,NSG=24000)
