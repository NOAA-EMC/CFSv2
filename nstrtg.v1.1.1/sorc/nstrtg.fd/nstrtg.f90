...
!input/output data file names
 ...
 character (len=6),  parameter :: fin_rtgmsk='rtgmsk'        ! land/water mask

 character (len=10), parameter :: fout_tf_grb='tf_rtg_grb'
 character (len=16), parameter :: fout_tf_grb_awips='tf_rtg_grb_awips'
 ...
 CALL W3TAGB('nsst_tf_to_rtg',2019,0,0,'emc')
...
!
 call get_mask(fin_rtgmsk,mask,nx,ny)
 masks = mask
 write(*,*) 'get mask from ',fin_rtgmsk
!
...
!
 call get_sal_clm(mask_ij,xlats_ij,xlons_ij,ny,nx,iy,im,id,ih,dsearch,miss_fill,bmiss,sal_clm)
!
...
!
 do j = 1, ny
    do i = 1, nx
       if ( mask(i,j) == 2 ) then
!         great lakes
          if ( xlats(j) > 41.0  .and. xlats(j) < 50.75 .and. &
               xlons(i) > 266.0 .and. xlons(i) < 285.0 ) then
             tf(i,j) = tzero
!         lake winnepeg, great bear lake, great slave lake, et al
          elseif ( xlats(j) > 50.0  .and. xlats(j) < 67.0 .and. &
                   xlons(i) > 235.0 .and. xlons(i) < 265.0 ) then
              tf(i,j) = tzero
          else
             if ( sal_clm(i,j) < 5.0 .or. sal_clm(i,j) > 50.0 ) then     ! non-water
