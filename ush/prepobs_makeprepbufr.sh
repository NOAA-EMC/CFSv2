...
#                   NOTE : This is required ONLY if the imported shell variable
#                          SENDDBN is "YES" (see below)
#     $HOMEobsproc         - formerly $HOMEobsproc_prep prior to WCOSS2
#                            string indicating directory path to generic prep
#                            subdirectories under version control
#                            (pre-WCOSS2, in production this is normally
#                            ${NWROOT}/obsproc_prep.vX.Y.Z where X.Y.Z is
#                            version number being used, usually the latest)
#                            WCOSS2: $NWROOT/obsproc.vX.Y.Z
#                          - formerly $HOMEobsproc_network prior to WCOSS2
#                            string indicating directory path to network-
...
#                   "$HOMEobsproc/fix/prepobs_errtable.cdas";
#                   if imported "NET=nam", default is
#                   "$HOMEobsproc/fix/prepobs_errtable.nam"
...
#                   "$HOMEobsproc/fix/prepobs_oiqc.oberrs.cdas"; 
#                   otherwise default is
#                   "$HOMEobsproc/fix/prepobs_oiqc.oberrs"
...
USHSYND=${USHSYND:-${HOMEobsproc}/ush}
USHGLERL=${USHGLERL:-${HOMEobsproc}/ush}
USHPREV=${USHPREV:-${HOMEobsproc}/ush}
USHCQC=${USHCQC:-${HOMEobsproc}/ush}
USHPQC=${USHPQC:-${HOMEobsproc}/ush}
USHVQC=${USHVQC:-${HOMEobsproc}/ush}
USHAQC=${USHAQC:-${HOMEobsproc}/ush}
USHOIQC=${USHOIQC:-${HOMEobsproc}/ush}

EXECPREP=${EXECPREP:-${HOMEobsproc}/exec}
PARMPREP=${PARMPREP:-${HOMEobsproc}/parm}
FIXPREP=${FIXPREP:-${HOMEobsproc}/fix}
DICTPREP=${DICTPREP:-${NWROOTp3}/decoders/decod_shared/dictionaries}

EXECSYND=${EXECSYND:-${HOMEobsproc}/exec}
PARMSYND=${PARMSYND:-${HOMEobsproc}/parm}
FIXSYND=${FIXSYND:-${HOMEobsproc}/fix}

GETGUESS=${GETGUESS:-YES}
if [ "$GETGUESS" = 'YES' ]; then
   USHGETGES=${USHGETGES:-${HOMEobsproc}/ush}
...
if [ "$RUN" = 'gdas' -o "$RUN" = 'gfs' ]; then
   ...
   PRVT=${PRVT:-$HOMEobsproc/fix/prepobs_errtable.cdas}
elif [ "$NET" = 'nam' ]; then
   PRVT=${PRVT:-$HOMEobsproc/fix/prepobs_errtable.nam}
else
...
if [ "$NET" = 'cdas' ]; then
   OIQCT=${OIQCT:-$HOMEobsproc/fix/prepobs_oiqc.oberrs.cdas}
else
   OIQCT=${OIQCT:-$HOMEobsproc/fix/prepobs_oiqc.oberrs}
fi
TIMEIT=${TIMEIT:-""}
