...
! $Revision$
!
! abstract: contains routines to decompose the model grid
!   among multiple mpi tasks (and vice versa)
!
! program history log:
...
!
! subprogram:  gaussian mpi setup
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: for gaussian grids, break up the grid along
!   latitude bands.
!
! program history log:
! ????        gayno     - initial version
...
!
!   input:
!     nlon       - # grid points in "i" direction
!     nlat       - # grid points in "j" direction
!     lonsperlat - # grid points in "i" direction for
!                  thinned grids
!
!   output: nonoe
!
...
!$$$ subroutine documentation block
!
! subprogram:  nam mpi setup
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: for nam grids, break up the grid into squares.
!
! program history log:
! ????        gayno     - initial version
!
...
!
!   input:
!     imdl       - # grid points in "i" direction
!     jmdl       - # grid points in "j" direction
!
!   output: none
!
! files: none
...
!$$$ subroutine documentation block
!
! subprogram:  gather - integer version
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: gathers integer subgrid data from each task 
!   (array patch) and places it in an array defined for
!   model grid (full_2d).
!
! program history log:
! ????        gayno     - initial version
...
!
!   input:
!     nlon       - total # grid points in "i" direction
!     nlat       - total # grid points in "j" direction
!     patch      - data on each task
!
!   output: 
!     full_2d    - data on full model grid
!
! files: none
!
! condition codes: none
...
!$$$ subroutine documentation block
!
! subprogram:  gather - floating point version
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: gathers real subgrid data from each task 
!   (array patch) and places it in an array defined for
!   model grid (full_2d).
!
! program history log:
...
!$$$ subroutine documentation block
!
! subprogram:  scatter, integer version
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: scatters integer data on the full model grid (full_2d)
!   to each mpi task (array patch).
!
! program history log:
! ????        gayno     - initial version
!
...
!$$$ subroutine documentation block
!
! subprogram:  scatter, real version
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: scatters real data on the full model grid (full_2d)
!   to each mpi task (array patch).
!
! program history log:
! ????        gayno     - initial version
