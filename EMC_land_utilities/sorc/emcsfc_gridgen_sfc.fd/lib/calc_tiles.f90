...
! $Revision$
!
! subprogram:  calculate tiles
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: compute the number of tiles - the idea the each grid
!   box may have multiple categories of fields such as soil type
!   or vegetation type.
!
! program history log:
...
!   input:
!     dominate_cat         - dominate category in grid box
!     percent_each_cat     - percent of grib box covered by a category
!     percent_each_group   - percent of grib box covered by a category
!                            grouping
!     num_categories       - number of categories
!     num_group            - number of cateory groupings
!     cat_groups           - the group each category is a member of
!     max_tiles            - maximum possible number of tiles
!     max_tiles_grid       - maximum number of tiles at each grid box
!     remaining_tot_tiles  - remaining number of tiles at each grid box
!
!   output:
!     remaining_tot_tiles  - remaining number of tiles at each grid box
!                            after additional tiles are set by this routine.
!     num_tiles            - number of tiles at each grid box.
!     tile_output          - data structure that holds the category and
!                            percent category for each tile at each grid box
!
! files: none
!
! condition codes: none
!
! remarks: See code comments for more details.
!
!$$$
...
!   print*,'- WARNING: more than max at i/j ',i,j,tile_output(i,j,1:max_tiles)
