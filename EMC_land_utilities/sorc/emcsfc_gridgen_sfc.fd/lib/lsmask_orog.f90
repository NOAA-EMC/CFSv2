...
! $Revision$
!
! abstract: routines that create land mask and terrain, 
!           including gravity wave drag fields, on the model grid.
!
! program history log:
! ????        gayno     - initial version
!
! usage: use lsmask_orog
!
! remarks: none
!
!$$$
...
!
! subprogram:  lsmask_orog_driver
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: driver routine to compute landmask and terrain.
!
! program history log:
! ????        gayno     - initial version
!
...
! files: 
!   input:
!     - landmask in grib 1 (optional)
!     - terrain/orography in grib 1 (optional)
!
!   output:
!     - landmask in grib 1 or grib 2 (when computed)
!     - terrain/orography in grib 1 or grib 2 (when computed)
!
! condition codes: 
!   1 - bad open or read of grib 1 landmask
!   2 - must input preprocessed landmask or raw data source
!   3 - bad open or read of grib 1 terrain/orog
!   4 - must input preprocessed terrain/orog or raw data source
!   5 - bad write of landmask grib file
!   6 - bad write of terrain/orog grib file
!
! remarks: user may choose to read in a precomputed landmask
!   and terrain on the model grid instead of computing it
!   here. (for example, if the user wants to just compute
!   vegetation type or some other field on an existing
!   model grid.)
!
!$$$
 ...
 use program_setup, only        : smooth, num_smooth_passes1, num_smooth_passes2, &
                                  smooth_bndy, thinned, orog_file, lsmask_file, &
 ...
 real, allocatable             :: lat_all(:,:), lon_all(:,:)
...
!-----------------------------------------------------------------------
 ...
 USE_EXT_MASK: if ( index(lsmask_file, ".grb ") > 0 ) then
   ...
   if (myrank == 0) then
     ...
     call baopenr (19, lsmask_file, ier)
     if (ier /= 0) then
       print*,'- FATAL ERROR: BAD OPEN, IRET IS ', ier
     ...
     jpds(5) = 81
     lskip   = -1
     call getgb(19, 0, (imdl*jmdl), -1, jpds, jgds, &
                numpts, lskip, kpds, kgds, lbms_src, dummy, ier)
     if (ier /= 0) then
       print*,"- FATAL ERROR: BAD DEGRIB OF DATA. IRET IS ", ier
       call mpi_abort(mpi_comm_world, 1, ier)
     end if
     call baclose(19, ier)
 ...
 elseif (len_trim(lsmask_file) > 0) then
   ...
   elseif (domain_type == "bgrid" .and. .not. lsmask_tiles) then
     ...
     allocate (dummy(imdl,jmdl))
     call gather(lsmask, imdl, jmdl, dummy)
     allocate (lat_all(imdl,jmdl))
     call gather(lat_mdl, imdl, jmdl, lat_all)
     allocate (lon_all(imdl,jmdl))
     call gather(lon_mdl, imdl, jmdl, lon_all)
     print*,"- REMOVE SMALL LAKES."
     call remove_lakes_bgrid(lat_all, lon_all, dummy)
     call scatter(lsmask, imdl, jmdl, dummy)
     deallocate (dummy, lat_all, lon_all)

   end if LAKES

 else

   print*,'- FATAL ERROR: MUST INPUT RAW OR PREPROCESSED LAND MASK DATA'
...
!     always do this last
   ...
   print*,'- FATAL ERROR: MUST INPUT RAW OR PREPROCESSED OROGRAPHY DATA'
   call lsmask_orog_cleanup
...
!-----------------------------------------------------------------------
 ...
 if (myrank == 0) then
   ...
   if (ier /= 0) then
     print*,"- FATAL ERROR GRIBBING LAND/SEA MASK, IER IS ", ier
     call mpi_abort(mpi_comm_world, 5, ier)
   end if
...
!-----------------------------------------------------------------------
 ...
 MULTIPLE_TILES : if (max_orog_tiles == 1) then
   ...
   if(grib2) then
     call grib2_init(gfld)
     gfld%discipline = 0  ! meteo products
     gfld%ipdtmpl(1) = 3  ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 6  ! oct 11; parameter, geometric hgt
     gfld%idrtmpl    = 0
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING OROGRAPHY, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
...
!-----------------------------------------------------------------------
 ...
 if (orog_gwd_tiles) then
   ...
   if(grib2) then
     call grib2_init(gfld)
     gfld%discipline = 0  ! meteological products
     gfld%ipdtmpl(1) = 3  ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 7  ! oct 11; parameter, stnd dev
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 1  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING STND DEV OF OROG, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   ...
   if(grib2) then
     ...
     gfld%idrtmpl(3) = 1  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
! this is a max field, which requires a different pdt (sec 4)
     gfld%ipdtnum=15      ! template 4.15 (extreme value)
     pdtsave=gfld%ipdtmpl ! save current values
     deallocate(gfld%ipdtmpl)   
     gfld%ipdtlen=18      ! new template requires 18 values
     allocate(gfld%ipdtmpl(gfld%ipdtlen))
     gfld%ipdtmpl(1:15)=pdtsave
     gfld%ipdtmpl(16)=2   ! oct 35, maximum
     gfld%ipdtmpl(17)=0   ! oct 36, max calc from source
     gfld%ipdtmpl(18)=255 ! oct 37, # pts used in processing, set to missing
   else
     kpds(5) = 221
   endif
   ...
   if (myrank == 0) then
     if(grib2) then
       call putgb2(lunout,gfld,ier)
       gfld%ipdtnum=0   ! revert to pdt 4.0
       deallocate(gfld%ipdtmpl)
       gfld%ipdtlen=15
       allocate(gfld%ipdtmpl(gfld%ipdtlen))
       gfld%ipdtmpl=pdtsave
     else
       call putgb (lunout, (imdl*jmdl), kpds, kgds, lbms, &
                   dummy, ier)
     endif
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING MAXIMUM HEIGHT, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3  ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 22  ! oct 11; parameter, slope or orog
     gfld%idrtmpl    = 0
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING SIGMA, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     ...
     gfld%ipdtmpl(2) = 213  ! oct 11; parameter, convexity
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING CONVEXITY, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3  ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 24  ! oct 11; parameter, aniostrophy
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING GAMMA, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3  ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 21  ! oct 11; parameter, angle orography
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING THETA, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3   ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 214  ! oct 11; parameter; oa - west
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING OA1, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3   ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 215  ! oct 11; parameter; oa - south
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING OA2, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3   ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 216  ! oct 11; parameter; oa - southwest
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING OA3, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3   ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 217  ! oct 11; parameter; oa - northwest
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING OA4, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3   ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 218  ! oct 11; parameter; ol - west
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING OL1, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3   ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 219 ! oct 11; parameter; ol - south
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING OL2, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3   ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 220 ! oct 11; parameter; ol - southwest
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING OL3, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
   ...
   if(grib2) then
     gfld%ipdtmpl(1) = 3   ! oct 10; parameter category, mass
     gfld%ipdtmpl(2) = 221 ! oct 11; parameter; ol - northwest
     gfld%idrtmpl    = 0
     gfld%idrtmpl(3) = 3  ! decimal scaling factor
     gfld%fld=reshape(dummy, (/imdl*jmdl/) )
   else
   ...
   if (myrank == 0) then
     ...
     if (ier /= 0) then
       print*,"- FATAL ERROR GRIBBING OL4, IER IS ", ier
       call mpi_abort(mpi_comm_world, 6, ier)
     end if
   end if
...
!$$$ subroutine documentation block
!
! subprogram:  calc_orog_bilinear
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: compute orography using bilinear interpolation.
!
! program history log:
! ????        gayno     - initial version
!
...
! files: 
!    input:
!      - raw orography source data 
!
!    output: none
!
! condition codes: 
!   1 - bad read or open of raw orography data file
!
! remarks: none.
!
!$$$
...
!----------------------------------------------------------------------
 ...
 if (ierr /= 0) then
   print*,'- FATAL ERROR: BAD OPEN, IERR IS: ', ierr
 ...
 if (ierr /= 0) then
   print*,'- FATAL ERROR: BAD READ, IERR IS: ', ierr
   call mpi_abort(mpi_comm_world, 1, ierr)
 endif
...
!$$$ subroutine documentation block
!
! subprogram:  calc_orog
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: compute orography using an area averaging
!   technique.  also compute gravity wave drag fields.
!
! program history log:
! ????        gayno     - initial version
!
...
! condition codes: 
!   1 - bad read or open of raw orography data file
!   2 - undefined map projection
!   3 - no source data within a model grid box
!
! remarks: none.
!
!$$$
 ...
 use ll2xy_util_polar, only : ll2xy_polar
...
!----------------------------------------------------------------------
 ...
 elseif (trim(domain_type) == "polar") then  ! polar stereographic
   ...
   do i = istart_src, iend_src
     ...
     call ll2xy_polar(orient_lon_mdl, dx_mdl, dy_mdl, hemi_mdl,  &
                      lat_src, lon_src, xgrid, ygrid)
 ...
 elseif (trim(domain_type) == "egrid") then  ! e-grid
   ...
   do i = istart_src, iend_src
     ...
     if (i > isrc) iii = i - isrc
     lon_src = lon_11_src + (iii-1)*dlon_src
     call ll2xy_egrid(lat_src, lon_src, imdl, &
 ...
 elseif (trim(domain_type) == "bgrid") then ! b-grid
   ...
   do i = istart_src, iend_src
     ...
     if (i > isrc) iii = i - isrc
     lon_src = lon_11_src + (iii-1)*dlon_src
     call ll2xy_bgrid(lat_src, lon_src, nearest_i, nearest_j)
     if (nearest_i >= istart_mdl .and. nearest_i <= iend_mdl .and. &
         nearest_j >= jstart_mdl .and. nearest_j <= jend_mdl) then
       i_wrt_mdl_grid(i,j)=nearest_i
       j_wrt_mdl_grid(i,j)=nearest_j
   ...
   print*,'- FATAL ERROR: UNDEFINED MAP PROJECTION.'
 ...
 do j = jstart_mdl, jend_mdl
  do i = istart_mdl, iend_mdl_4_loops(j)
    if (count(i,j) == 0) then
      print*,'- FATAL ERROR: NO SOURCE DATA WITHIN MODEL GRID AT I/J: ', i,j
...
!$$$ subroutine documentation block
!
! subprogram:  calc_lsmask_bilinear
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: compute landmask using bilinear averaging
!
! program history log:
! ????        gayno     - initial version
!
...
! files: 
!    input:
!      - raw landmask source data 
!
!    output: none
!
! condition codes: 
!   1 - bad read or open of raw landmask data file
!
! remarks: none.
!
!$$$
...
!$$$ subroutine documentation block
!
! subprogram:  calc_lsmask_aavg
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: compute landmask using area averaging
!
! program history log:
! ????        gayno     - initial version
!
...
!   1 - bad read or open of raw landmask data file
!   2 - undefined map projection
!   3 - no source data within model grid box
!
! remarks: none.
!
!$$$
...
!$$$ subroutine documentation block
!
! subprogram:  lsmask_orog_cleanup
!   prgmmr: gayno          org: w/np2           date: ????
!
...
!$$$ subroutine documentation block
!
! subprogram:  waterfalls
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract:  ensure mask and terrain are consistent at water points by
!  'flattening' the water.
!
! program history log:
! ????        gayno     - initial version
!
! usage:  call routine with these arguments:
!   input:
!     lsmask   - model landmask; water is zero
!
!   output:
!     orog     - model orography after flattening
!
! files: none
!
! condition codes: none
...
!$$$ subroutine documentation block
!
! subprogram:  waterfalls, "e" grid version
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract:  ensure mask and terrain are consistent at water points by
!  'flattening' the water.  "e" grid version
!
! program history log:
! ????        gayno     - initial version
!
...
!$$$ subroutine documentation block
!
! subprogram:  remove lakes, "e" grid version
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract:  remove isolated lakes, "e" grid version
!
! program history log:
! ????        gayno     - initial version
!
...
!
!   input:
!     lsmask   - model landmask; water is zero; land is one
!
!   output:
!     landmask - model landmask after lake removal
!
! files: none
!
! condition codes: none
...
!$$$
 ...
 subroutine remove_lakes_bgrid(lat_all, lon_all, lsmask)
!$$$ subroutine documentation block
!
! subprogram:  remove lakes, "b" grid version
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract:  remove isolated lakes, "b" grid version
!
! program history log:
! ????        gayno     - initial version
! 2015-06-15  gayno     - logic to remove or not remove
!                         small lake within the FLAKE
!                         SST climatology domain.
!
! usage:  call routine with these arguments:
!
!   input:
...
!
!   output:
!     lsmask   - model landmask after lake removal
!
! files: none
!
! condition codes: none
...
!$$$
 ...
 use program_setup, only : imdl, jmdl, lsmask_flake

 implicit none

 integer                :: count, i, j

 real, intent(in   )    :: lat_all(imdl,jmdl), lon_all(imdl,jmdl)
 real, intent(inout)    :: lsmask(imdl,jmdl)
...
! When true, dont remove lakes in flake sst domain, which is
! between 25 and 75N latitude and 199 and 305 longitude.

     if(lsmask_flake) then
       if (lat_all(i,j) > 25.0 .and. lat_all(i,j) < 75.0) then
         lon = lon_all(i,j)
         if (lon < 0.0) lon = lon + 360.0
         if (lon > 199.0 .and. lon < 305.0) then
           cycle
         endif
       endif
     endif
...
!$$$ subroutine documentation block
!
! subprogram:  coastlines, "e" grid version
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract:  at each water point, check the elevation of the surrounding
!    land points. if a land point is lower than the water point, raise
...
!
!   output:
!     orog     - model orography after correction
!
! files: none
!
! condition codes: none
...
!$$$ subroutine documentation block
!
! subprogram:  coastlines, non "e" grid version
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract:  at each water point, check the elevation of the surrounding
!    land points. if a land point is lower than the water point, raise
...
!$$$ subroutine documentation block
!
! subprogram:  remove lakes, gaussian grids
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract:  remove isolated lakes for gaussian grid.  can work with
