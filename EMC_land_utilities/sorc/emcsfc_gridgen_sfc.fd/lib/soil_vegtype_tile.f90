...
! $Revision$
!
! abstract: contains routines to determine soil and
!   vegetation type on the model grid.
!
! program history log:
! ????        gayno     - initial version
!
! usage: use soil_vegtype_tile
!
! remarks: none
!
!$$$
...
!
! subprogram:  soil_vegtype_driver
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: driver routine to compute soil type and
!   vegetation type on the model grid and grib the result.
!
! program history log:
! ????        gayno     - initial version
!
...
!$$$ subroutine documentation block
!
! subprogram:  tile_driver
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: driver routine to compute soil type and
!   vegetation type on the model grid and grib the result.
!   routine computes the dominate category and user selected
!   number of tiles - allowing for multiple categories per model
!   grid box.
!
! program history log:
! ????        gayno     - initial version
...
!   inputs:
!     source file         - path/name of input source soil/veg file
!     max_num_categories  - maximum number of categories
!     max_tiles           - maximum number of tiles
!     tile_threshold      - minimum % grid box coverage required
!                           for a tile
!     default_category    - default value given to model points
!                           not located near source data.
!     num_groups          - number of category groups
!     groups_dum          - the group # each category is a member of
!     kpds5               - parameter indicator; 224-soil type, 225-veg type
!
!   outputs: none
!
! files: none
...
!-----------------------------------------------------------------------
 ...
 if (iret /= 0) then
   print*,'- FATAL ERROR: BAD OPEN, IERR IS ', iret
...
!-----------------------------------------------------------------------
 ...
 if (myrank == 0) then
   ...
   if (iret /= 0) then
     print*,"FATAL ERROR: BAD OPEN OF OUTPUT GRIB FILE: ", trim(fngrib), " IRET IS ", iret
     call mpi_abort(mpi_comm_world, 1, iret)
   end if
 end if
...
!-----------------------------------------------------------------------
 ...
 if (myrank == 0) then
   ...
   if (iret /= 0) then
     print*,"FATAL ERROR: BAD WRITE OF FILE: ", trim(fngrib), " IRET IS ", iret
     call mpi_abort(mpi_comm_world, 1, iret)
   end if
 end if
...
!-----------------------------------------------------------------------
 ...
 9000 print*,'- FATAL ERROR: BAD READ OF SOURCE FILE, IERR IS ', iret
 call mpi_abort(mpi_comm_world, 1, iret)
