...
!
! subprogram:  smooth/desmooth
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: Apply the smoother-desmoother from the MM5 program TERRAIN
!   (found in smther.F) to array (for non-e grids).
!
! program history log:
! ????        gayno     - initial version
!
! usage:  call routine the following arguments:
!
!   inputs:
!      array       - data to be smoothed
!      lsmask      - land/sea mask
!      npass       - number of smoothing passes
!      start_x/y/z - starting x/y/z index of 'array'
!      end_x/y/z   - ending x/y/z index of 'array'
!
!   outputs:
!      array       - data after smoothing
!
! files: none
!
...
!$$$ subroutine documentation block
!
! subprogram:  smooth/desmooth for 'e' grids
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: Apply the smoother-desmoother from the MM5 program TERRAIN
!   (found in smther.F) to array (for e grids).
!
! program history log:
! ????        gayno     - initial version
!
...
!      start_x/y/z - starting x/y/z index of 'array'
!      end_x/y/z   - ending x/y/z index of 'array'
!      hflag       - set to one for the 'h' grid
!
!   outputs:
!      array       - data after smoothing
!
...
!$$$ subroutine documentation block
!
! subprogram:  smdhld_egrid
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: Apply 5-point smoothing along lateral boundaries.
!   Chop mountain peaks over entire domain. "E" grid version.
!
! program history log:
! ????        gayno     - initial version
!
...
!$$$ subroutine documentation block
!
! subprogram:  inner_bound_egrid
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: Apply extra smoothing along inner boundary, 
!   "e" grid version.
!
! program history log:
! ????        gayno     - initial version
...
!
!   inputs:
!      ime         - 'i' dimension of grid
!      jme         - 'j' dimension of grid
!
!   outputs:
!      h           - smoothed terrain data
!
! files: none
!
! condition codes: none
...
!
! abstract: Apply 5-point smoothing along lateral boundaries.
!   Chop mountain peaks over entire domain. non-"E" grid version.
!
! program history log:
! ????        gayno     - initial version
!
...
!
!   inputs:
!     imdl   - x dimension limit 
!     jmdl   - j dimension limit
!     H      - input/output topography field
!     lsmask - input land/sea mask (0=water, 1=land)
!     NSMTH  - number of mountain chopping passes.
!              if zero, only the lateral boundaries 
!              are smoothed.  lateral boundaries 
!              always smoothed with 12 passes.
!
!   outputs:
!     H      - input/output topography field
!
...
! condition codes: none
!
! remarks: the number of outer row/cols smoothed is controlled
!   by variable "lines".
!
!$$$
      implicit none
...
!   inputs:
!      lsmask      - land/sea mask
!      imdl        - 'i' dimension of grid
!      jmdl        - 'j' dimension of grid
!
!   outputs:
!      h           - smoothed terrain data
!
