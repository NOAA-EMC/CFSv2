...
!
! subprogram:  roughness
!   prgmmr: gayno          org: w/np2           date: ????
!
! $Revision$
!
! abstract: driver routine to compute roughness length
!   on the model grid and grib the result.  May be 
!   computed from an external z0 dataset or from a 
!   vegetation type-based lookup table.  Tables exist
!   for the usgs and igbp vegetation categories.
!   For the lookup table option, the vegetation types
!   on the model grid must computed by routine
!   soil_vegtype_driver.
!
! program history log:
...
!
!   output:
!     - roughness length on model grid (grib 1 or grib 2)
!
! condition codes: none
!   1 - vegetation type on model grid must be available 
!       for lookup table option
!   2 - invalid choice of vegetation type category for
!       lookup table option
!   3 - bad open roughness grib file
!   4 - bad write roughness grib file
!
! remarks: none.
!
!$$$
...
!-----------------------------------------------------------------------
 ...
 if (trim(roughness_file) == "usgs" .or. trim(roughness_file) == "igbp") then
   ...
   if (.not. allocated(dominate_veg_cat)) then
     print*,"- FATAL ERROR: VEGETATION TYPE MUST BE AVAILABLE."
   ...
   elseif (trim(roughness_file) == "igbp") then
     ...
     print*,"- FATAL ERROR: INVALID CHOICE OF VEGETATION TYPE: ", trim(roughness_file)
...
! nmmb adds 0.1 meters to roughness for all but cat 15 - perm ice/snow.
   ...
   if (myrank == 0) then
     ...
     if (iret /= 0) then
       print*,'- FATAL ERROR: BAD WRITE OF FILE:', trim(output_file), ' IRET IS ', iret
