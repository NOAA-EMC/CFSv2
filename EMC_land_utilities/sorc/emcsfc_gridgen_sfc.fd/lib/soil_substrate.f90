...
!
! subprogram:  soil substrate temperature
!   prgmmr: gayno          org: w/np2           date: ????
!
! $Revision$
!
! abstract: routine to compute soil substrate temperature
!   on the model grid and grib the result.  based on the average
!   low-level air temperature adjusted for terrain.
!
! program history log:
...
! files:
!   input:
!     - global file of climatological air temperature (binary, non-grib)
!
!   output:
!     - substrate temperature on the model grid (grib 1 or grib 2)
!
! condition codes: 
!    1  - bad open, input temperature file
!    2  - unrecognized model domain type
!    3  - undefined substrate temperature at a model point
!    4  - bad open, output grib file
!    5  - bad write, output grib file
!    6  - bad read, input temperature file
!
! remarks: none.
!
!$$$
...
!-----------------------------------------------------------------------
 ...
 if (iret /= 0) then
   print*,'- FATAL ERROR: BAD OPEN: ', iret
     ...
     UNDEFINED : if (soilt_mdl(i,j) < (default_value + 0.001)) then
       ...
       if (tavg > 0.0) then
         ...
         if (soilt_mdl(i,j) < (default_value + 0.001)) then
           print*,'- FATAL ERROR: UNDEFINED SUBSTRATE TEMPERATURE AT POINT: ',i,j
...
!----------------------------------------------------------------------
 ...
 9000 print*,'- FATAL ERROR READING DATA. IRET IS: ', iret
