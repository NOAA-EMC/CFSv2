...
!
! subprogram:  interpolate tiles
!   prgmmr: gayno          org: w/np2           date: ????
!
! $Revision$
!
! abstract: interpolate categorical fields to the model
!   grid and determine the dominate category, percentage
!   of each category, and percentage of each category
!   group.
!
! program history log:
...
!
!   input:
!     lon_11_src     - longitude of source grid corner pt
!     lat_11_src     - latitude of source grid corner pt
!     srcdat         - data on source grid
!     isrc           - full 'i' dimension of source grid
!     istart_src     - starting 'i' index on source grid 
!                      corresponding to model grid
!     iend_src       - ending 'i' index on source grid 
!                      corresponding to model grid
!     jstart_src     - starting 'j' index on source grid 
!                      corresponding to model grid
!     jend_src       - ending 'j' index on source grid 
!                      corresponding to model grid
!     dlon_src       - 'i' resolution of source grid
!     dlat_src       - 'j' resolution of source grid
!     num_categories - number of categories
!     num_groups     - number of category groupings
!     water_category - water category (to be ignored)
!     default_category - default value when interpolation
!                        cant find nearby source grid value
!     tile_threshold - minimum % grid box coverage required
!                      for a category to be a tile
!     cat_groups     - the group membership of each category
!
!   output:
!     out_cat        - % grid box coverage of each category
!     out_group      - % grid box coverage of each group
!     dominate_cat   - dominate category for each grid box
!
! files: none
!
! condition codes: all fatal
!   1 - unrecognized grid projection
!   2 - the percentage of each category is less than the minimum
!       threshold for a tile.
!
! remarks: none.
!
!$$$
...
!------------------------------------------------------------------
 ...
 elseif (trim(domain_type) == 'bgrid') then
   ...
   do i = istart_src, iend_src
      ...
      if (category /= water_category) then
        srclat = lat_11_src + (j-1)*dlat_src
        srclon = lon_11_src + (iii-1)*dlon_src
        call ll2xy_bgrid(srclat, srclon, nearest_i, nearest_j)
        if (nearest_i >= istart_mdl .and. nearest_i <= iend_mdl .and. &
            nearest_j >= jstart_mdl .and. nearest_j <= jend_mdl) then
          count_cat(nearest_i,nearest_j,category) = count_cat(nearest_i,nearest_j,category) + 1
          group                   = cat_groups(category)
 ...
 elseif (trim(domain_type) == "latlon") then
   do j = jstart_src, jend_src
     srclat = lat_11_src + (j-1)*dlat_src
     nearest_j = nint((srclat - lat_first_mdl) / dy_mdl + 1.0)
     if (nearest_j >= jstart_mdl .and. nearest_j <= jend_mdl) then
       do i = istart_src, iend_src
         category = srcdat(i,j)
         if (category /= water_category) then
           srclon = lon_11_src + (i-1)*dlon_src
           nearest_i = nint(1.0 + mod((srclon-lon_first_mdl)+3600,360.0)/dx_mdl)
   ...
   print*,'- FATAL ERROR IN ROUTINE INTERP_TILES: UNRECOGNIZED DOMAIN TYPE: ', trim(domain_type)
...
! say 10% or less. 
         print*,'FATAL ERROR: no cats above threshold at ',i,j
