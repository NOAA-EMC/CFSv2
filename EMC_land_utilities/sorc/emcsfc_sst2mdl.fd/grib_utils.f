...
! files:
!   input files:        
!      - fort.11 (file to be checked)
!
!   output files: none
!
! condition codes:
!      40 - bad open of fort.11 (fatal)
!
! remarks: none.
!
!$$$
 ...
 print*,"- OPEN AND CHECK FILE TYPE OF: ", trim(file_name)
...
!
! condition codes:
!    50 - bad conversion to the grib 1 gds because grid
!         type unrecognized.  fatal.
!
! remarks: none.
!          
!$$$
 ...
 elseif (igdtnum.eq.1) then    ! Rotated Lat/Lon grid
   ...
   print*,'- FATAL ERROR: BAD CONVERSION TO GRIB2 GDT.'
...
!
! condition codes:
!    47 - unrecognized grid projection type.  fatal
!
...
!      idrstmplen                   - length of grib2 drt
!                                     template.
...
!      idrsnum                      - grib2 drs template number
!      ipdsnum                      - grib2 pds template number
!      idrstmpl                     - grib2 drs template array
...
! Section 4 - product definition section
 ...
 ipdstmpl(1)= 3         ! oct 10; parameter category, surface properties
! note!! to use a parmeter number >= 192 you must set the local table to '1'
 ipdstmpl(2)= 0         ! oct 11; parameter, water temperature
