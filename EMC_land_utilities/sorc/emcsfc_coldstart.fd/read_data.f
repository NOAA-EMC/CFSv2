...
!
! module:  read_data
!   prgmmr: gayno          org: w/np2           date: 2005-08-03
!
! $Revision$
!
! abstract: contains routines to read surface fields on the
!   input grid.
!
! program history log:
...
! usage: use read_data
!
! remarks: the input surface fields are stored in the
!   "data_input" data structure.
!
!$$$
...
!
! subprogram:  read_input_file
!   prgmmr: gayno          org: w/np2           date: 2005-08-03
!
! abstract: driver routine to read the surface fields from the
!   input grid.
!
! program history log:
! 2005-08-03  gayno     - initial version
!
! usage:  call read_input_file with no arguments
!
! files: none
!
! condition codes: 
!   77 - unrecognized input grid file type
!
! remarks: none
!
!$$$
...
!$$$ subroutine documentation block
!
! subprogram:  read_input_file_gfs
!   prgmmr: gayno          org: w/np2           date: 2005-08-03
!
! abstract: read input land-surface states from gfs restart file.
!
! program history log:
! 2005-08-03  gayno     - initial version
!
! usage:  call read_input_file_gfs with no arguments
!
! files:
!   input:
!     - gfs restart file
...
!$$$
 ...
 if (header%ivs < 200501) then
   print*,"- FATAL ERROR: PROGRAM NOT TESTED WITH OLD STYLE GFS FILE."
   call w3tage('COLDSTART')
...
!-----------------------------------------------------------------------
 ...
 9000 print*,"- FATAL ERROR OPENING GFS RESTART FILE. STATUS IS: ", iret
 call w3tage('COLDSTART')
 call mpi_abort(mpi_comm_world, 89, iret)

 9100 print*,"- FATAL ERROR READING GFS RESTART FILE. STATUS IS: ", iret
 call w3tage('COLDSTART')
...
!$$$ subroutine documentation block
!
! subprogram:  read_input_file_nems
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: read input land-surface states from a nemsio restart file.
!
! program history log:
! ????-??-??  gayno     - initial version
!
! usage:  call read_input_file_nems with no arguments
!
! files:
!   input:
!     - nemsio restart file
...
!
! condition codes: 
!   33 - error reading nemsio restart file
!
! remarks: none
!
!$$$
 ...
 if (iret /= 0) then
   print*,'- WARNING: CLOSE FAILED, IRET IS:',iret
 endif
...
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: the outer two rows/columns of the nmm restart file 
!   are hosed, so replace the data with nearest neighbors
!   from the grid interior.
!
! program history log:
! ????-??-??  gayno     - initial version
!
...
!   input:
!      i/jmdl - number of model rows/columns
!
!   output:
!      var - the variable to fix (real*8)
!
! files: none
!
...
!
!   output:
!      var - the variable to fix (integer)
!
! files: none
!
! condition codes: none
...
!$$$ subroutine documentation block
!
! subprogram:  read_input_file_edas
!   prgmmr: gayno          org: w/np2           date: ????
!
! abstract: read input land-surface states from a 12km edas file.
!
! program history log:
! ????-??-??  gayno     - initial version
!
! usage:  call read_input_file_edas with no arguments
!
! files:
!   input:
!     - edas restart file
...
!
! remarks: the file is 4-byte ints/floats
!
!$$$
...
!-----------------------------------------------------------------------
 ...
 open(19, file=trim(input_file), form="unformatted", status='old', &
         iostat=istat, err=9100)

 read(19, iostat=istat, err=9000, end=9000) ifhr, ihrst, idat(1), idat(2), idat(3)
...
!----------------------------------------------------------------------
 ...
 read(19, iostat=istat, err=9000, end=9000) landmask ! 0-seaice/land, 1-open sea
 read(19, iostat=istat, err=9000, end=9000) icemask  ! 0-land/open sea, 1-seaice
 ...
 read(19, iostat=istat, err=9000, end=9000) dum  ! lats
 read(19, iostat=istat, err=9000, end=9000) dum  ! lons
...
!-----------------------------------------------------------------------
 ...
 read(19, iostat=istat, err=9000, end=9000) dum  
 allocate (data_input%mxsnow_alb(imdl_input,jmdl_input))
 data_input%mxsnow_alb = dum

 where (data_input%lsmask < 1.0) data_input%mxsnow_alb = 0.0

 read(19, iostat=istat, err=9000, end=9000) dum  ! espr

 allocate(data_input%substrate_temp(imdl_input,jmdl_input))
 read(19, iostat=istat, err=9000, end=9000) dum  ! substrate t
 data_input%substrate_temp = dum

 read(19, iostat=istat, err=9000, end=9000) dum  ! gffc
...
!-----------------------------------------------------------------------
 ...
 read(19, iostat=istat, err=9000, end=9000) wbd, sbd, tlm0d, tph0d, dlmd, dphd
...
!-----------------------------------------------------------------------
 ...
 allocate(data_input%veg_type(imdl_input,jmdl_input))
 read(19, iostat=istat, err=9000, end=9000) idum
...
!-----------------------------------------------------------------------
 ...
 read(19, iostat=istat, err=9000, end=9000) psfc  ! psfc
...
!-----------------------------------------------------------------------
 ...
 read(19, iostat=istat, err=9000, end=9000) dum ! akhs
...
!-----------------------------------------------------------------------
 ...
 read(19, iostat=istat, err=9000, end=9000) dum ! qs
...
!-----------------------------------------------------------------------
 ...
 allocate(data_input%soilm_tot(imdl_input,jmdl_input,nsoil_input))
 read(19, iostat=istat, err=9000, end=9000) dum3d 
...
!-----------------------------------------------------------------------
 ...
 allocate(data_input%soilm_liq(imdl_input,jmdl_input,nsoil_input))
 read(19, iostat=istat, err=9000, end=9000) dum3d ! liq soilm
...
!-----------------------------------------------------------------------
 ...
 read(19, iostat=istat, err=9000, end=9000) dum   ! albedo, inc snow effects
...
9000 continue
 print*,"- ** FATAL ERROR READING EDAS DATA **"
 print*,"- ** ISTAT IS ", istat
 call w3tage("COLDSTART")
 call mpi_abort(mpi_comm_world, 60, istat)

9100 continue
 print*,"- ** FATAL ERROR OPENING EDAS FILE **"
 print*,"- ** ISTAT IS ", istat
 call w3tage("COLDSTART")
