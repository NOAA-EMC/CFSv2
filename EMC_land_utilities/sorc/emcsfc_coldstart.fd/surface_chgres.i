...
!
! abstract: a collection of routines to interpolate surface fields
...
!
! condition codes:
!    - if error, argument "iret" is non-zero
!
! remarks:
...
!
!$$$
 use ll2xy_utils, only       : ll2xy_egrid, ll2xy_bgrid
     ...
     if (ipts(ii,jj) > imdl_input) then
       if (ipts(ii,jj) > (imdl_input+cushion)) then
         if (merge) then
           ...
           print*,'- FATAL ERROR: OUTPUT GRID IS OUTSIDE INPUT GRID'
           iret=20
           return
         endif
       else
...
!-----------------------------------------------------------------------
 ...
 if (allocated(output%greenfrc_min) .and. allocated(output%greenfrc_max)) then
   ...
   elseif (.not. allocated(input%greenfrc_max) .and.  &
     ...
     print*,"- FATAL ERROR: INPUT FILE DOES NOT HAVE MAX/MIN GREENNESS."
     iret = 99
     return
   else
...
!-----------------------------------------------------------------------
 if (allocated (output%slope_type)) then
   ...
   elseif (.not. allocated (input%slope_type)) then
     print*,"- FATAL ERROR: INPUT FILE DOES NOT HAVE SLOPE TYPE."
     iret = 99
     return
...
!-----------------------------------------------------------------------
 if (allocated (output%mxsnow_alb)) then
   ...
   elseif (.not. allocated(input%mxsnow_alb)) then
     print*,"- FATAL ERROR: INPUT FILE DOES NOT HAVE MAX SNOW ALBEDO."
     iret = 99
     return
   else  ! interpolate from input grid
...
!
! input files:
!   unit 41    configuration namelists
!
! condition codes:
!    iret = 1;  invalid output grid soil type
!    iret = 2;  invalid output grid vegetation type
!    iret = 3;  invalid input grid vegetation type
!    iret = 4;  invalid land ice option for input grid with land ice
!    iret = 5;  invalid land ice option for input grid without land ice
!    iret = 6;  error opening configuration namelist
!    iret = 7;  error reading configuration namelist
!    iret = 9;  unrecognized input grid map projection.
!    iret = 10; unrecognized output grid map projection.
...
!$$$
 ...
 read(41, nml=soil_parameters, iostat=istat, err=910)
 read(41, nml=veg_parameters, iostat=istat, err=910)
 read(41, nml=options, iostat=istat, err=910)
 close(41)
 ...
 elseif (kgds_input(1) == 205) then  ! b-grid
   ...
   print*,"- FATAL ERROR: UNRECOGNIZED INPUT GRID MAP PROJECTION"                   
   iret=9
   return
 end if
 ...
 elseif (kgds_output(1) == 3) then
   ...
   print*,"- FATAL ERROR: UNRECOGNIZED OUTPUT GRID MAP PROJECTION"                   
   iret=10
   return
 end if
...
!-----------------------------------------------------------------------
 ...
 select case (trim(soil_src_output))
   ...
   case("statsgo")
     soil_type_ice=16
   case default
     print*,'- FATAL ERROR: INVALID OUTPUT GRID SOIL SOURCE ',trim(soil_src_output)
     iret=1
     return
 end select
 ...
 select case (trim(veg_src_output))
   ...
   case("igbp")
     veg_type_ice=15
   case default
     print*,'- FATAL ERROR: INVALID OUTPUT GRID VEG SOURCE ',trim(veg_src_output)
     iret=2
     return
 end select
 ...
 select case (trim(veg_src_input))
   ...
   case default
     print*,'- FATAL ERROR: INVALID INPUT GRID VEG SOURCE ',trim(veg_src_input)
     iret=3
     return
 end select
 ...
 if (landice_opt == 1 .or. landice_opt == 3) then 
   ...
   do i = 1, imdl_input
     if (input%lsmask(i,j) > 0.0 .and. input%soilm_tot(i,j,1) > 0.99) then
       print*,"- FATAL ERROR: LANDICE OPTION OF ", landice_opt, " IS NOT VALID WHEN"
...
!
! files:
!   input:  (surface fields are grib 1 or grib 2)
!     snow free albedo
!     greenness fraction
!     maximum snow albedo
!     slope type
...
!
! condition codes:
!   fatal error indicated by non-zero value of argument "ierr"
!
! subprograms called:
...
!$$$
 ...
 open(41, iostat=istat, err=900)
 read(41, nml=optional_output_fields, iostat=istat, err=910)
 close(41)
...
!-----------------------------------------------------------------------
 ...
 if (len_trim(soil_type_output_file) > 0) then
   ...
   if (isgrib == 0) then
     print*,'- FATAL ERROR: FILE MUST BE GRIB1 OR GRIB2 FORMAT'
     iret=-1
     return
   endif
   ...
   if (isgrib == 1) then
     ...
     call read_grib_data(soil_type_output_file, 224, dummy, &
                         ijmdl_output, iret)
     if (iret /= 0) then
       print*,'- FATAL ERROR DEGRIBBING OUTPUT GRID SOIL TYPE'
       return
     end if
 ...
 if (len_trim(veg_type_output_file) > 0) then
   ...
   if (isgrib == 1) then
     ...
     call read_grib_data(veg_type_output_file, 225, dummy, &
                         ijmdl_output, iret)
     if (iret /= 0) then
       print*,'- FATAL ERROR DEGRIBBING OUTPUT GRID VEG TYPE'
       return
     end if
 ...
 if (len_trim(slope_type_output_file) > 0) then
   ...
   if (isgrib /= 1) then
     print*,'- FATAL ERROR: FILE MUST BE GRIB1 FORMAT'
     iret=-1
     return
   endif
   ...
   if (iret /= 0) then
     print*,'- FATAL ERROR DEGRIBBING OUTPUT GRID SLOPE TYPE'
     return
   end if
 ...
 if (len_trim(mxsnow_alb_output_file) > 0) then
   ...
   if (isgrib == 1) then
     ...
     if (iret /= 0) then
       print*,'- FATAL ERROR DEGRIBBING OUTPUT GRID MAX SNOW ALBEDO'
       return
     end if
...
!-----------------------------------------------------------------------
 ...
 if (len_trim(greenfrc_output_file) > 0) then
   ...
   elseif (isgrib == 2) then
     ...
     print*,"- ** FATAL ERROR: GREENNESS FILE IS NOT GRIB1 OR GRIB2 **"
     iret = 1
   endif

   if (iret /= 0) then
     print*,'- ** FATAL ERROR DURING INVENTORY OF GREENNESS CLIMO FILE **'
     return
   end if
   ...
   if (iret /= 0) then
     print*,'- FATAL ERROR DEGRIBBING OUTPUT GRID GREENNESS FRACTION.'
     return
   end if
...
!-----------------------------------------------------------------------
 ...
 if (len_trim(snow_free_albedo_output_file) > 0) then
   ...
   elseif (isgrib == 2) then
     ...
     print*,"- FATAL ERROR: FILE IS NOT GRIB1 OR GRIB2"
     iret = 1
   endif

   if (iret /= 0) then
     print*,'- FATAL ERROR DURING INVENTORY OF SNOW-FREE ALBEDO'
     return
   end if
   ...
   if (iret /= 0) then
     print*,'-FATAL ERROR DEGRIBBING OUTPUT GRID SNOW-FREE ALBEDO'
     return
   end if
...
!-----------------------------------------------------------------------
 ...
 if (len_trim(z0_output_file) > 0) then
   ...
   if (iret /= 0) then
     print*,'- FATAL ERROR DURING INVENTORY OF OUTPUT GRID ROUGHNESS LENGTH'
     return
   end if
   ...
   if (iret /= 0) then
     print*,'- FATAL ERROR DEGRIBBING OUTPUT GRID ROUGHNESS LENGTH'
     return
   end if
...
! files:
!   input:
!     - the grib 2 file to be inventoried
!
!   output: none
!
! condition codes:
!   status code "ierr" is non-zero if error reading the grib 2 file
!
! remarks: none.
!
!$$$
...
! model grid.
 ...
 if (num_recs > 0) then
   ...
   print*,"- ** FATAL ERROR, FILE CONTAINS ", num_recs, " RECORDS OF DATA."
   ierr = 1
 endif
...
! files:
!   input:
!     - the grib 2 data file
!
!   output: none
!
! condition codes:
!    status code "ierr" is non-zero when there is a problem
!    opening or reading the grib 2 file.
!
! remarks: none.
!
!$$$
 ...
 if (ierr /= 0) then
   print*,"- ** FATAL ERROR OPENING FILE: ", ierr
   return
 endif
...
! records we want (stored in rec_nums)
 ...
 do jj = 1, num_recs
   ...
   if (ierr /= 0) then
     print*,'- ** FATAL ERROR DEGRIBBING RECORD'
     call grib2_free(gfld)
     exit
   endif
